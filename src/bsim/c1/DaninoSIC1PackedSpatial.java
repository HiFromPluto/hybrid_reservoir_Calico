...
/**
 * Gate C1: frozen packed patch of Object B cells coupled through a
 * conservative spatial He field on the P0 mask. NOT_FIG4B.
 *
 * Mechanics OFF. Identity clock is SI-scaled (not the P0 25950 s clock).
 * Membrane uses the local voxel; C0 algebra is local, not global.
 */
public final class DaninoSIC1PackedSpatial {
    ...
    public static final int PATCH_I0 = 97;
    public static final int PATCH_I1 = 101;
    public static final int PATCH_J0 = 22;
    public static final int PATCH_J1 = 26;
    ...
    public DaninoSIC1PackedSpatial(BSimRandom rng) {
        this(N_CELLS, V_CELL, D1_SPATIAL, rng);
    }

    public DaninoSIC1PackedSpatial(int n, double vCell, double d1Spatial, BSimRandom rng) {
        if (n != N_CELLS) {
            throw new IllegalArgumentException("C1 identity N is frozen at " + N_CELLS + " NOT_FIG4B");
        }
        if (Math.abs(vCell - V_CELL) > 0.0) {
            throw new IllegalArgumentException("C1 v_cell is frozen at " + V_CELL + " NOT_FIG4B");
        }
        if (Math.abs(d1Spatial - D1_SPATIAL) > 0.0) {
            throw new IllegalArgumentException("C1 D1_spatial is frozen at " + D1_SPATIAL + " NOT_FIG4B");
        }
        this.n = n;
        this.vCell = vCell;
        this.d = D_LOCAL;
        ...
        this.unusedRng = rng;
        this.fluid = DaninoPocketGeometry.buildFluidMask();
        this.maskSha = DaninoPocketGeometry.maskSha256(fluid);
        if (!MASK_SHA256.equals(maskSha)) {
            throw new IllegalStateException(
                    "P0 mask SHA mismatch: " + maskSha + " vs " + MASK_SHA256 + " NOT_FIG4B");
        }
        if (Math.abs(d - 0.5) > 1e-15) {
            throw new IllegalStateException("C1 local d is " + d + " not 0.5 NOT_FIG4B");
        }
        this.vi = new int[n];
        this.vj = new int[n];
        this.vk = new int[n];
    ...
    public Trajectory integrate(
        ...
        sim.setBound(
                DaninoPocketGeometry.BUS_LENGTH_UM,
                DaninoPocketGeometry.POCKET_LY_UM + DaninoPocketGeometry.BUS_WIDTH_UM,
                DaninoPocketGeometry.POCKET_LZ_UM);
        ...
        int[] boxes = {DaninoPocketGeometry.NX, DaninoPocketGeometry.NY, DaninoPocketGeometry.NZ};
    ...
    private void recordSample(
        ...
        for (int ii = 0; ii < DaninoPocketGeometry.NX; ii++) {
            for (int jj = 0; jj < DaninoPocketGeometry.NY; jj++) {
