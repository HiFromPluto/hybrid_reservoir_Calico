package BSimReservoirPlanSweepS3;
...
/**
 * Sweep Step 3: luminescence lag tau_L scout on the frozen claim dish.
 * Position (500,250,5), rate 1.28e8, K=1.6, tau_R=15. Only tau_L changes.
 * ALPHA_LUX and DELTA_LUX both read from config and must stay equal.
 */
public final class BSimReservoirPlanSweepS3 {
    ...
    /** Dedicated death-input source; not AC0/AC1/AC2. Do not move. */
    static final Vector3d ACID_SOURCE_POSITION = new Vector3d(300, 375, 5);
    /** AHL inject position. Default claim (500,250,5). Not used for attractants. */
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close SweepS3 output", e);
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        assertClaimAhlPosition();
        assertClaimAhlRate();
        assertAlphaEqualsDelta();
        System.out.printf(Locale.US,
                "SweepS3 AHL source=(%.4f, %.4f, %.4f) rate=%.0f acid=(%.4f, %.4f, %.4f) alpha=%.10f delta=%.10f tau_L=%.4f%n",
                ...
                ALPHA_LUX, DELTA_LUX, 1.0 / DELTA_LUX);
        resetStaticState();
        ...
        try {
            ...
            throw new RuntimeException("Cannot open SweepS3 output", e);
        }
        ...
        System.out.printf(Locale.US,
                "SweepS3 complete arm=%s ahl=(%.1f,%.1f,%.1f) rate=%.0f alpha=%.10f delta=%.10f summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
    ...
    static void assertClaimAhlRate() {
        if (Math.abs(AHL_SOURCE_RATE - 128000000.0) > 1e-3) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS3 AHL rate must be 128000000, printed %.6f", AHL_SOURCE_RATE));
        }
    }
    ...
    static void assertAlphaEqualsDelta() {
        if (Math.abs(ALPHA_LUX - DELTA_LUX) > 1e-15) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS3 alpha=%.12g != delta=%.12g (lag change requires alpha=delta=1/tau_L)",
                    ALPHA_LUX, DELTA_LUX));
        }
        if (!(DELTA_LUX > 0)) {
            throw new IllegalStateException("STOP SweepS3 delta must be positive");
        }
    }

    static void assertClaimAhlPosition() {
        if (Math.abs(AHL_SOURCE_POSITION.x - 500.0) > 1e-9
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS3 AHL must be (500, 250, 5), printed (%.4f, %.4f, %.4f)",
                    AHL_SOURCE_POSITION.x, AHL_SOURCE_POSITION.y, AHL_SOURCE_POSITION.z));
        }
    }
    ...
    static void writeSourcePositions(File directory) throws IOException {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write(String.format(Locale.US, "ahl.x=%.6f%n", AHL_SOURCE_POSITION.x));
            w.write(String.format(Locale.US, "ahl.y=%.6f%n", AHL_SOURCE_POSITION.y));
            w.write(String.format(Locale.US, "ahl.z=%.6f%n", AHL_SOURCE_POSITION.z));
            w.write(String.format(Locale.US, "ahl.rate=%.6f%n", AHL_SOURCE_RATE));
            ...
            w.write(String.format(Locale.US, "tau_L=%.6f%n", 1.0 / DELTA_LUX));
            w.write(String.format(Locale.US, "acid.x=%.6f%n", ACID_SOURCE_POSITION.x));
            w.write(String.format(Locale.US, "acid.y=%.6f%n", ACID_SOURCE_POSITION.y));
            w.write(String.format(Locale.US, "acid.z=%.6f%n", ACID_SOURCE_POSITION.z));
            w.write(String.format(Locale.US, "att0.x=%.6f%n", AC_POSITIONS[0].x));
            w.write(String.format(Locale.US, "att0.y=%.6f%n", AC_POSITIONS[0].y));
            w.write(String.format(Locale.US, "att0.z=%.6f%n", AC_POSITIONS[0].z));
            w.write(String.format(Locale.US, "att2.x=%.6f%n", AC_POSITIONS[2].x));
            w.write(String.format(Locale.US, "att2.y=%.6f%n", AC_POSITIONS[2].y));
            w.write(String.format(Locale.US, "att2.z=%.6f%n", AC_POSITIONS[2].z));
            w.write("note=SweepS3: AHL locked (500,250,5) rate 1.28e8; K=1.6; tau_R=15; only tau_L via alpha=delta\n");
        }
    }
    ...
    static void writeRunStatus(OutputOwner output) {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write(String.format(Locale.US,
                    "arm=%s%nseed=%d%nahl.x=%.6f%nahl.y=%.6f%nahl.z=%.6f%nahl.rate=%.6f%nalpha=%.12g%ndelta=%.12g%ntau_L=%.6f%nsummary_rows=%d%nsample_rows=%d%nvoxel_rows=%d%n",
                    ARM.name().toLowerCase(Locale.ROOT), RNG_SEED,
                    AHL_SOURCE_POSITION.x, AHL_SOURCE_POSITION.y, AHL_SOURCE_POSITION.z,
                    AHL_SOURCE_RATE, ALPHA_LUX, DELTA_LUX, 1.0 / DELTA_LUX,
