...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanStage6.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanStage6.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanStage6.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanStage6.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanStage6.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanStage6.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanStage6.pHFromAcidMm(
                            BSimReservoirPlanStage6.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanStage6.ReservoirBacterium bacterium : bacteria) {
