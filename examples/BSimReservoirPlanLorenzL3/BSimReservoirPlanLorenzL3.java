package BSimReservoirPlanLorenzL3;
...
/**
 * Track E3 Lorenz L3 living scout on the frozen Plan Stage 6 / BenchA
 * claim dish. One AHL channel, CENTER / FLOW=0, 75 s pulse / 300 s
 * window. Mechanisms are not retuned. BenchA / BenchA2 GATE_EVIDENCE
 * files are not edited. Selected clock is k=10 AUTO_X only.
 */
public final class BSimReservoirPlanLorenzL3 {
    ...
    static String OUTPUT_DIR = "results/l3_k10_driven_seed101";
    static String INPUT_AHL_FILE = "input_ahl_lorenz_k10.txt";
    static String INPUT_ACID_FILE = "input_acid_held05_200.txt";
    static String RUN_LABEL = "l3_k10_driven";
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close Bench A output", e);
            }
        }
    }

    public static void main(String[] args) {
        String configPath = args.length > 0 ? args[0] : "sim_config_l3.properties";
        loadConfig(configPath);
        if (args.length > 1 && "preview".equals(args[1])) HEADLESS = false;
        resetStaticState();
        System.out.printf(Locale.US,
                "LORENZ_L3 dish=%.0fx%.0fx%.0f FLOW_SPEED=%.1f AHL_source=(500,250,5) "
                        + "AHL_rate=%g arm=%s seed=%d num.windows=%d pulse=%.0f window=%.0f%n",
                BOUND_X, BOUND_Y, BOUND_Z, FLOW_SPEED, AHL_SOURCE_RATE,
                ARM.name().toLowerCase(Locale.ROOT), RNG_SEED, NUM_WINDOWS,
                PULSE_DURATION, WINDOW_DURATION);
        if (FLOW_SPEED != 0.0)
            throw new IllegalStateException("LORENZ_L3 claim dish requires FLOW_SPEED=0");
        ...
        try {
            ...
            throw new RuntimeException("Cannot open Lorenz L3 output", e);
        }
        ...
        System.out.printf(Locale.US,
                "LORENZ_L3 complete arm=%s FLOW_SPEED=%.1f AHL_finite=%s "
                        + "summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
                ARM.name().toLowerCase(Locale.ROOT), FLOW_SPEED,
                ahlFinite ? "true" : "false",
            ...
            if (output.voxels != null) {
                ...
                if (ARM == Arm.BROWNIAN) {
                    ...
                    for (double v : r.ahlUm) {
                        if (!Double.isFinite(v)) ahlFinite = false;
    ...
    static void resetStaticState() {
        ...
        voxelAnalyzer = null; nextId = 0; cumulativeDeaths = 0; ahlFinite = true;
    }
}
