...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanLorenzL3.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanLorenzL3.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanLorenzL3.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanLorenzL3.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanLorenzL3.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanLorenzL3.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanLorenzL3.pHFromAcidMm(
                            BSimReservoirPlanLorenzL3.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanLorenzL3.ReservoirBacterium bacterium : bacteria) {
