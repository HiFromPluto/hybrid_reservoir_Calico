...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanNarmaC1.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanNarmaC1.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanNarmaC1.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanNarmaC1.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanNarmaC1.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanNarmaC1.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanNarmaC1.pHFromAcidMm(
                            BSimReservoirPlanNarmaC1.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanNarmaC1.ReservoirBacterium bacterium : bacteria) {
