package BSimReservoirPlanNarmaC1;
...
/**
 * C1 independent-input NARMA-10 on the frozen HybridDish claim dish.
 * Clone of Narma10b. Only the AHL u file changes per trajectory.
 * Mechanisms are not retuned. Track B / E5 / Stage99 are not started.
 */
public final class BSimReservoirPlanNarmaC1 {
    ...
    static String OUTPUT_DIR = "results/c1_driven_traj01_seed111";
    static String INPUT_AHL_FILE = "input_ahl_narma200_traj01.txt";
    static String INPUT_ACID_FILE = "input_acid_held05_200.txt";
    static String RUN_LABEL = "c1_driven";
    static String STOCHASTIC_REPLICATE = "driven_traj01_seed111";
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close NarmaC1 output", e);
            }
        }
    }

    public static void main(String[] args) {
        String configPath = args.length > 0 ? args[0] : "sim_config_c1.properties";
        loadConfig(configPath);
        if (args.length > 1 && "preview".equals(args[1])) HEADLESS = false;
        System.out.printf(Locale.US,
                "NarmaC1 layout=CENTER FLOW=0 NO_FLUX ahl_source=(%.1f, %.1f, %.1f) "
                        + "acid_source=(%.1f, %.1f, %.1f) num.windows=%d arm=%s seed=%d ahl_file=%s%n",
                AC_POSITIONS[1].x, AC_POSITIONS[1].y, AC_POSITIONS[1].z,
                ACID_SOURCE_POSITION.x, ACID_SOURCE_POSITION.y, ACID_SOURCE_POSITION.z,
                NUM_WINDOWS, ARM.name().toLowerCase(Locale.ROOT), RNG_SEED, INPUT_AHL_FILE);
        resetStaticState();
        ...
        try {
            ...
            throw new RuntimeException("Cannot open NarmaC1 output", e);
        }
        ...
        System.out.printf(Locale.US,
                "NarmaC1 complete arm=%s summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
    ...
    static void writeRunStatus(OutputOwner output) {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write(String.format(Locale.US,
                    "arm=%s%nseed=%d%nlayout=CENTER%nflow=0%nboundary=NO_FLUX%n"
                            + "ahl_source=500,250,5%nacid_source=300,375,5%n"
                            + "num_windows=%d%nahl_file=%s%n"
                            + "summary_rows=%d%nsample_rows=%d%nvoxel_rows=%d%n",
                    ARM.name().toLowerCase(Locale.ROOT), RNG_SEED, NUM_WINDOWS, INPUT_AHL_FILE,
