...
public final class VoxelAnalyzer {
    public static final class VoxelReadout {
        public final double[] attractant, repellent, ahl, meanLuxI, meanQ, fractionQAboveHalf;
        public final int[] density, births, deaths;

        VoxelReadout(double[] attractant, double[] repellent, double[] ahl,
                     int[] density, double[] meanLuxI, double[] meanQ,
                     double[] fractionQAboveHalf, int[] births, int[] deaths) {
    ...
    public VoxelReadout analyze(BSimChemicalField attractantField,
                                ...
                                List<BSimReservoirPlanStage3.ReservoirBacterium> bacteria) {
        int total = getStateVoxels();
        double[] attractant = new double[total], repellent = new double[total];
        double[] ahl = new double[total], luxISum = new double[total];
        double[] qSum = new double[total], qAbove = new double[total];
        ...
        for (BSimReservoirPlanStage3.ReservoirBacterium bacterium : bacteria) {
            ...
            qSum[id] += bacterium.getQ();
            if (bacterium.getQ() > .5) qAbove[id]++;
        }

        double[] meanLuxI = new double[total], meanQ = new double[total];
        double[] fractionQAboveHalf = new double[total];
        for (int i = 0; i < total; i++) {
            if (density[i] > 0) {
                meanLuxI[i] = luxISum[i] / density[i];
                meanQ[i] = qSum[i] / density[i];
                fractionQAboveHalf[i] = (double) qAbove[i] / density[i];
            }
        }
        return new VoxelReadout(attractant, repellent, ahl, density, meanLuxI,
                meanQ, fractionQAboveHalf, birthAccum.clone(), deathAccum.clone());
    }
