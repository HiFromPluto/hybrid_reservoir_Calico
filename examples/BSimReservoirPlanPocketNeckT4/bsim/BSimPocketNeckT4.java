...
/**
 * PocketNeck-T4 growth-on N(t) / spillover. Clone of T3 chip (wall-mask,
 * bus advection, cyan AC visual). Growth on; HybridDish p_removal clamp
 * at K=4000; acid off. No NARMA, no LuxI, no J_max retune.
 */
public final class BSimPocketNeckT4 {
    enum Arm { LIVE_W20_GROWTH, LIVE_W100_GROWTH }
    ...
    static boolean CLAMP_OFF = false;
    ...
    static Arm ARM = Arm.LIVE_W20_GROWTH;
    static String OUTPUT_DIR = "results/runs/LIVE_W20_GROWTH";
    static String RUN_LABEL = "live_w20_growth";
    ...
    public static final class ReservoirBacterium extends BSimBacterium {
        ...
        public void action() {
            super.action();
            if (!GROWTH_ON)
                throw new IllegalStateException("T4 growth must stay on");
            if (ACID_CELL_PRODUCTION_RATE != 0.0)
                throw new IllegalStateException("T4 acid cell production must stay off");
            if (CLAMP_OFF)
                throw new IllegalStateException("T4 clamp must stay on (HybridDish p_removal)");
            ...
            if (experimentRng.nextDouble() < pRemoval)
                markRemoval(RemovalCause.CLAMP);
        }
    ...
    public static void main(String[] args) {
        ...
        if (Math.abs(DT - DT_FROZEN) > 1e-12)
            throw new IllegalArgumentException("T4 freezes dt=0.02; got " + DT);
        if (!SMOKE && Math.abs(DURATION - DURATION_FROZEN) > 1e-12)
            throw new IllegalArgumentException("T4 freezes duration=18000 s; got " + DURATION);
        if (!GROWTH_ON)
            throw new IllegalArgumentException("T4 growth.on must be true");
        if (CLAMP_OFF)
            throw new IllegalArgumentException("T4 clamp.off must be false (HybridDish p_removal)");
        if (CARRYING_CAPACITY != CARRYING_CAPACITY_FROZEN)
            throw new IllegalArgumentException("T4 carrying capacity must be 4000");
        if (RNG_SEED != 101)
            throw new IllegalArgumentException("T4 rng.seed must be 101");
        if (INITIAL_POP != 50)
            throw new IllegalArgumentException("T4 initial.pop must be 50");
        if (ACID_CELL_PRODUCTION_RATE != 0.0)
            throw new IllegalArgumentException("T4 acid cell production must be 0");
        ...
        boolean necked = ARM == Arm.LIVE_W20_GROWTH;
        ...
        System.out.printf(Locale.US, "PocketNeckT4 done arm=%s dir=%s%n", ARM.name(), OUTPUT_DIR);
    }
        ...
        try (BufferedWriter w = new BufferedWriter(new FileWriter(new File(dir, "summary.csv")))) {
            ...
            w.write(String.format(Locale.US,
                    "%s,STEP_ON,bsim_java_chip,POPULATION,%s,%.1f,%.4f,%.1f,%.1f,%.1f,%.2f,"
                            ...
                            + "%.9g,%s,%s,%d,%d,%d,%d,%d,%d,%s,%d,%.9g,%.9g,%s,%.9g,%.9g,%.9g,%.9g,"
                    ...
                    "true", CARRYING_CAPACITY, cellR, cellL, "false",
    ...
    static void printTheory() {
        ...
        System.out.println("PocketNeck-T4 THEORY (growth-on N(t) / spillover; T3 chip)");
        ...
        System.out.printf(Locale.US,
                "  GROWTH_RATE=%.6g  T_gen=%.1f s  K_clamp=%d  p_removal HybridDish%n",
                GROWTH_RATE, T_REMOVAL, CARRYING_CAPACITY_FROZEN);
        System.out.println("  Hypothesis: LIVE_W20_GROWTH occupancy ALIVE (AC still writes).");
        ...
        System.out.println("  LIVING_DECISION=STOP_AFTER_T4_POPULATION");
    }
    ...
    static void loadConfig(String path) {
        ...
        CLAMP_OFF = Boolean.parseBoolean(p.getProperty("clamp.off", "false"));
        ...
        if (!CELLS)
            throw new IllegalArgumentException("T4 arms are living; cells must be true");
        ...
        if (Math.abs(growthRateCfg - GROWTH_RATE) > 1e-12)
            throw new IllegalArgumentException("T4 growth.rate must stay 4*pi/1800; got " + growthRateCfg);
    }
