...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanSweepS4.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanSweepS4.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanSweepS4.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanSweepS4.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanSweepS4.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanSweepS4.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanSweepS4.pHFromAcidMm(
                            BSimReservoirPlanSweepS4.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanSweepS4.ReservoirBacterium bacterium : bacteria) {
