package BSimReservoirPlanSweepS4;
...
/**
 * Sweep Step 4: box vs plume scout on the frozen claim dish.
 * Bounds, grid, source positions, pop, and clamp scale. AHL rate stays
 * 1.28e8. K=1.6, tau_L=1500 (alpha=delta). Readout stays 20x10 + 4x2.
 */
public final class BSimReservoirPlanSweepS4 {
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close SweepS4 output", e);
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            throw new RuntimeException("Cannot open SweepS4 output", e);
        }
        ...
        System.out.printf(Locale.US,
                "SweepS4 complete arm=%s bound=(%.1f,%.1f,%.1f) grid=%dx%dx%d ahl=(%.1f,%.1f,%.1f) rate=%.0f alpha=%.10f delta=%.10f summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
                ...
                AHL_SOURCE_RATE, ALPHA_LUX, DELTA_LUX,
    ...
    static void assertClaimAhlRate() {
        if (Math.abs(AHL_SOURCE_RATE - 128000000.0) > 1e-3) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS4 AHL rate must be 128000000, printed %.6f", AHL_SOURCE_RATE));
        }
    }
    ...
    static void assertClaimAcidRate() {
        if (Math.abs(ACID_SOURCE_RATE - 2.0e11) > 1.0) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS4 acid rate must be 2e11, printed %.6g", ACID_SOURCE_RATE));
        }
    }
    ...
    static void assertAlphaEqualsDelta() {
        if (Math.abs(ALPHA_LUX - DELTA_LUX) > 1e-15) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS4 alpha=%.12g != delta=%.12g", ALPHA_LUX, DELTA_LUX));
        }
        if (!(DELTA_LUX > 0)) {
            throw new IllegalStateException("STOP SweepS4 delta must be positive");
        }
        if (Math.abs(ALPHA_LUX - 1.0 / 1500.0) > 1e-12
                || Math.abs(DELTA_LUX - 1.0 / 1500.0) > 1e-12) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS4 alpha=delta must be 1/1500, printed alpha=%.12g delta=%.12g",
                    ALPHA_LUX, DELTA_LUX));
        }
    }
    ...
    static void assertReadoutFrozen() {
        if (STATE_X != 20 || STATE_Y != 10 || STATE_Z != 1
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS4 readout must stay 20x10x1 + 4x2x1, printed %dx%dx%d / %dx%dx%d",
    ...
    static void assertBoundZ() {
        if (Math.abs(BOUND_Z - 10.0) > 1e-12) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS4 bound.z must be 10, printed %.6f", BOUND_Z));
        }
    }
    ...
    static void assertDtFrozen() {
        if (Math.abs(DT - 0.05) > 1e-12) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS4 dt must be 0.05, printed %.6f", DT));
        }
    }
    ...
    static void assertCfl() {
        ...
        if (DT > DT_CFL_MAX) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS4 dt=%.6f exceeds CFL dt_max=%.6f (dx=%.6f dy=%.6f). Do not lower dt.",
    ...
    static void requireInside(String name, double x, double y, double z) {
        if (x < 0 || x > BOUND_X || y < 0 || y > BOUND_Y || z < 0 || z > BOUND_Z) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS4 %s=(%.4f, %.4f, %.4f) outside [0,bound]=(%.1f, %.1f, %.1f)",
    ...
    static void printGeometryBanner() {
        System.out.printf(Locale.US,
                "SweepS4 bound.x=%.6f bound.y=%.6f bound.z=%.6f%n", BOUND_X, BOUND_Y, BOUND_Z);
        System.out.printf(Locale.US,
                "SweepS4 grid.x=%d grid.y=%d grid.z=%d dx=%.6f dy=%.6f CFL_dt_max=%.6f dt=%.6f%n",
                GRID_X, GRID_Y, GRID_Z, FIELD_DX, FIELD_DY, DT_CFL_MAX, DT);
        System.out.printf(Locale.US,
                "SweepS4 field.ahl.source.x=%.6f field.ahl.source.y=%.6f field.ahl.source.z=%.6f rate=%.0f%n",
                AHL_SOURCE_POSITION.x, AHL_SOURCE_POSITION.y, AHL_SOURCE_POSITION.z, AHL_SOURCE_RATE);
        System.out.printf(Locale.US,
                "SweepS4 field.acid.source.x=%.6f field.acid.source.y=%.6f field.acid.source.z=%.6f rate=%.6g%n",
                ACID_SOURCE_POSITION.x, ACID_SOURCE_POSITION.y, ACID_SOURCE_POSITION.z, ACID_SOURCE_RATE);
        System.out.printf(Locale.US,
                "SweepS4 field.att0.x=%.6f field.att0.y=%.6f field.att0.z=%.6f%n",
                ATT0_POSITION.x, ATT0_POSITION.y, ATT0_POSITION.z);
        System.out.printf(Locale.US,
                "SweepS4 field.att2.x=%.6f field.att2.y=%.6f field.att2.z=%.6f%n",
                ATT2_POSITION.x, ATT2_POSITION.y, ATT2_POSITION.z);
        System.out.printf(Locale.US,
                "SweepS4 initial.pop=%d carrying.capacity=%d luminescence.alpha=%.12g luminescence.delta=%.12g%n",
    ...
    static void writeSourcePositions(File directory) throws IOException {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write("note=SweepS4: box vs plume; AHL rate 1.28e8 not scaled with area; K=1.6; tau_L=1500; readout 20x10+4x2\n");
        }
        if (SMOKE_NOT_NARMA) {
            try (BufferedWriter w = new BufferedWriter(
