...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanSweepS5.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanSweepS5.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanSweepS5.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanSweepS5.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanSweepS5.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanSweepS5.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanSweepS5.pHFromAcidMm(
                            BSimReservoirPlanSweepS5.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanSweepS5.ReservoirBacterium bacterium : bacteria) {
