package BSimReservoirPlanSweepS5;
...
/**
 * Sweep Step 5: +x flow scout on the frozen claim dish.
 * FLOW_SPEED from config. Upwind-advect AHL, acid, attractant, and
 * repellent (Stage 6 scheme). Stokes +x on bacterium and BrownianParticle.
 * Claim box 1000x500x10, AHL (500,250,5), rate 1.28e8, K=1.6, tau_L=1500.
 */
public final class BSimReservoirPlanSweepS5 {
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close SweepS5 output", e);
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            throw new RuntimeException("Cannot open SweepS5 output", e);
        }
        ...
        System.out.printf(Locale.US,
                "SweepS5 complete arm=%s FLOW_SPEED=%.6f transit_s=%s Courant=%.6f bound=(%.1f,%.1f,%.1f) ahl=(%.1f,%.1f,%.1f) rate=%.0f summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
                ARM.name().toLowerCase(Locale.ROOT), FLOW_SPEED, formatTransit(TRANSIT_S),
                CHEMICAL_COURANT, BOUND_X, BOUND_Y, BOUND_Z,
    ...
    static void assertClaimAhlRate() {
        if (Math.abs(AHL_SOURCE_RATE - 128000000.0) > 1e-3) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 AHL rate must be 128000000, printed %.6f", AHL_SOURCE_RATE));
        }
    }

    static void assertClaimAcidRate() {
        if (Math.abs(ACID_SOURCE_RATE - 2.0e11) > 1.0) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 acid rate must be 2e11, printed %.6g", ACID_SOURCE_RATE));
        }
    }
    ...
    static void assertAlphaEqualsDelta() {
        if (Math.abs(ALPHA_LUX - DELTA_LUX) > 1e-15) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 alpha=%.12g != delta=%.12g", ALPHA_LUX, DELTA_LUX));
        }
        if (!(DELTA_LUX > 0)) {
            throw new IllegalStateException("STOP SweepS5 delta must be positive");
        }
        if (Math.abs(ALPHA_LUX - 1.0 / 1500.0) > 1e-12
                || Math.abs(DELTA_LUX - 1.0 / 1500.0) > 1e-12) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 alpha=delta must be 1/1500, printed alpha=%.12g delta=%.12g",
                    ALPHA_LUX, DELTA_LUX));
        }
    }
    ...
    static void assertReadoutFrozen() {
        if (STATE_X != 20 || STATE_Y != 10 || STATE_Z != 1
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 readout must stay 20x10x1 + 4x2x1, printed %dx%dx%d / %dx%dx%d",
    ...
    static void assertBoundZ() {
        if (Math.abs(BOUND_Z - 10.0) > 1e-12) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 bound.z must be 10, printed %.6f", BOUND_Z));
        }
    }
    ...
    static void assertDtFrozen() {
        if (Math.abs(DT - 0.05) > 1e-12) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 dt must be 0.05, printed %.6f", DT));
        }
    }
    ...
    static void assertCfl() {
        ...
        if (DT > DT_CFL_MAX) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 dt=%.6f exceeds CFL dt_max=%.6f (dx=%.6f dy=%.6f). Do not lower dt.",
    ...
    static void requireInside(String name, double x, double y, double z) {
        if (x < 0 || x > BOUND_X || y < 0 || y > BOUND_Y || z < 0 || z > BOUND_Z) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 %s=(%.4f, %.4f, %.4f) outside [0,bound]=(%.1f, %.1f, %.1f)",
    ...
    static void assertClaimBox() {
        if (Math.abs(BOUND_X - 1000.0) > 1e-9
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 bounds must be 1000x500x10, printed %.4f x %.4f x %.4f",
                    BOUND_X, BOUND_Y, BOUND_Z));
        }
        if (GRID_X != 50 || GRID_Y != 25 || GRID_Z != 1) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 grid must be 50x25x1, printed %dx%dx%d",
        ...
        if (Math.abs(AHL_SOURCE_POSITION.x - 500.0) > 1e-9
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 AHL must be (500, 250, 5), printed (%.4f, %.4f, %.4f)",
                    AHL_SOURCE_POSITION.x, AHL_SOURCE_POSITION.y, AHL_SOURCE_POSITION.z));
        }
        if (Math.abs(ACID_SOURCE_POSITION.x - 300.0) > 1e-9
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 acid must be (300, 375, 5), printed (%.4f, %.4f, %.4f)",
    ...
    static void assertFlowCourant() {
        if (FLOW_SPEED < 0) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 FLOW_SPEED must be >= 0, printed %.6f", FLOW_SPEED));
        }
        ...
        if (CHEMICAL_COURANT >= 1.0) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS5 chemical Courant=%.6f >= 1 (FLOW_SPEED=%.6f dt=%.6f dx=%.6f). Do not lower dt.",
                    CHEMICAL_COURANT, FLOW_SPEED, DT, FIELD_DX));
        }
    }
    ...
    static void printGeometryBanner() {
        System.out.printf(Locale.US,
                "SweepS5 bound.x=%.6f bound.y=%.6f bound.z=%.6f%n", BOUND_X, BOUND_Y, BOUND_Z);
        System.out.printf(Locale.US,
                "SweepS5 grid.x=%d grid.y=%d grid.z=%d dx=%.6f dy=%.6f CFL_dt_max=%.6f dt=%.6f%n",
                GRID_X, GRID_Y, GRID_Z, FIELD_DX, FIELD_DY, DT_CFL_MAX, DT);
        System.out.printf(Locale.US,
                "SweepS5 field.ahl.source.x=%.6f field.ahl.source.y=%.6f field.ahl.source.z=%.6f rate=%.0f%n",
                AHL_SOURCE_POSITION.x, AHL_SOURCE_POSITION.y, AHL_SOURCE_POSITION.z, AHL_SOURCE_RATE);
        System.out.printf(Locale.US,
                "SweepS5 field.acid.source.x=%.6f field.acid.source.y=%.6f field.acid.source.z=%.6f rate=%.6g%n",
                ACID_SOURCE_POSITION.x, ACID_SOURCE_POSITION.y, ACID_SOURCE_POSITION.z, ACID_SOURCE_RATE);
        System.out.printf(Locale.US,
                "SweepS5 field.att0.x=%.6f field.att0.y=%.6f field.att0.z=%.6f%n",
                ATT0_POSITION.x, ATT0_POSITION.y, ATT0_POSITION.z);
        System.out.printf(Locale.US,
                "SweepS5 field.att2.x=%.6f field.att2.y=%.6f field.att2.z=%.6f%n",
                ATT2_POSITION.x, ATT2_POSITION.y, ATT2_POSITION.z);
        System.out.printf(Locale.US,
                "SweepS5 initial.pop=%d carrying.capacity=%d luminescence.alpha=%.12g luminescence.delta=%.12g%n",
                INITIAL_POP, CARRYING_CAPACITY, ALPHA_LUX, DELTA_LUX);
        System.out.printf(Locale.US,
                "SweepS5 FLOW_SPEED=%.6f transit_s=%s chemical_Courant=%.6f CFL_dt_max_flow=%s%n",
                FLOW_SPEED, formatTransit(TRANSIT_S), CHEMICAL_COURANT, formatTransit(DT_CFL_MAX_FLOW));
        System.out.println("SweepS5 advect=AHL+acid+att+rep Stokes=bacterium+BrownianParticle");
    }
    ...
    static void writeSourcePositions(File directory) throws IOException {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write(String.format(Locale.US, "field.ahl.source.rate=%.6f%n", AHL_SOURCE_RATE));
            ...
            w.write(String.format(Locale.US, "field.acid.source.rate=%.6g%n", ACID_SOURCE_RATE));
            ...
            w.write("advect=ahl,acid,att,rep\n");
            w.write("stokes=bacterium,BrownianParticle\n");
            w.write("note=SweepS5: +x flow scout; claim box 1000x500x10; AHL (500,250,5) rate 1.28e8; K=1.6; tau_L=1500; Stage 6 upwind; no recycling\n");
        }
        if (SMOKE_NOT_NARMA) {
            try (BufferedWriter w = new BufferedWriter(
