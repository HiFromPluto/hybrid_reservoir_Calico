package BSimReservoirPlanSweepE03;
...
/**
 * Sweep E0.3: layout-by-flow living scout on the frozen claim dish.
 * FLOW_SPEED and AHL x from config. Upwind-advect AHL, acid, attractant,
 * and repellent (Stage 6 / SweepS5 scheme) only when FLOW_SPEED > 0.
 * Stokes +x on bacterium and BrownianParticle. Claim box 1000x500x10,
 * rate 1.28e8, K=1.6, tau_L=1500. Do not rerun 8 um/s.
 */
public final class BSimReservoirPlanSweepE03 {
    ...
    static String OUTPUT_DIR = "results/e03_center_f0p0_driven_seed111";
    static String INPUT_AHL_FILE = "input_ahl_narma200.txt";
    static String INPUT_ACID_FILE = "input_acid_held05_200.txt";
    static String RUN_LABEL = "e03_center_f0p0_driven";
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close SweepE03 output", e);
            }
        }
    }

    public static void main(String[] args) {
        String configPath = args.length > 0 ? args[0] : "sim_config_narma10b.properties";
        ...
        try {
            ...
            throw new RuntimeException("Cannot open SweepE03 output", e);
        }
        ...
        System.out.printf(Locale.US,
                "SweepE03 complete arm=%s FLOW_SPEED=%.6f transit_s=%s Courant=%.6f chemical_boundary=%s bound=(%.1f,%.1f,%.1f) ahl=(%.1f,%.1f,%.1f) rate=%.0f summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
                ARM.name().toLowerCase(Locale.ROOT), FLOW_SPEED, formatTransit(TRANSIT_S),
                CHEMICAL_COURANT, CHEMICAL_BOUNDARY, BOUND_X, BOUND_Y, BOUND_Z,
    ...
    static void assertClaimAhlRate() {
        if (Math.abs(CONFIGURED_AHL_SOURCE_RATE - 128000000.0) > 1e-3) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 AHL rate must be 128000000, printed %.6f", CONFIGURED_AHL_SOURCE_RATE));
        }
    }
    ...
    static void assertClaimAcidRate() {
        if (Math.abs(CONFIGURED_ACID_SOURCE_RATE - 2.0e11) > 1.0) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 acid rate must be 2e11, printed %.6g", CONFIGURED_ACID_SOURCE_RATE));
        }
    }
    ...
    static void assertAlphaEqualsDelta() {
        if (Math.abs(ALPHA_LUX - DELTA_LUX) > 1e-15) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 alpha=%.12g != delta=%.12g", ALPHA_LUX, DELTA_LUX));
        }
        if (!(DELTA_LUX > 0)) {
            throw new IllegalStateException("STOP SweepE03 delta must be positive");
        }
        if (Math.abs(ALPHA_LUX - 1.0 / 1500.0) > 1e-12
                || Math.abs(DELTA_LUX - 1.0 / 1500.0) > 1e-12) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 alpha=delta must be 1/1500, printed alpha=%.12g delta=%.12g",
                    ALPHA_LUX, DELTA_LUX));
        }
    }
    ...
    static void assertReadoutFrozen() {
        if (STATE_X != 20 || STATE_Y != 10 || STATE_Z != 1
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 readout must stay 20x10x1 + 4x2x1, printed %dx%dx%d / %dx%dx%d",
    ...
    static void assertBoundZ() {
        if (Math.abs(BOUND_Z - 10.0) > 1e-12) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 bound.z must be 10, printed %.6f", BOUND_Z));
        }
    }
    ...
    static void assertDtFrozen() {
        if (Math.abs(DT - 0.05) > 1e-12) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 dt must be 0.05, printed %.6f", DT));
        }
    }
    ...
    static void assertCfl() {
        ...
        if (DT > DT_CFL_MAX) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 dt=%.6f exceeds CFL dt_max=%.6f (dx=%.6f dy=%.6f). Do not lower dt.",
    ...
    static void requireInside(String name, double x, double y, double z) {
        if (x < 0 || x > BOUND_X || y < 0 || y > BOUND_Y || z < 0 || z > BOUND_Z) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 %s=(%.4f, %.4f, %.4f) outside [0,bound]=(%.1f, %.1f, %.1f)",
    ...
    static void assertClaimBox() {
        if (Math.abs(BOUND_X - 1000.0) > 1e-9
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 bounds must be 1000x500x10, printed %.4f x %.4f x %.4f",
                    BOUND_X, BOUND_Y, BOUND_Z));
        }
        if (GRID_X != 50 || GRID_Y != 25 || GRID_Z != 1) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 grid must be 50x25x1, printed %dx%dx%d",
                    GRID_X, GRID_Y, GRID_Z));
        }
        boolean allowedAhlX = Math.abs(AHL_SOURCE_POSITION.x - 200.0) < 1e-9
                || Math.abs(AHL_SOURCE_POSITION.x - 500.0) < 1e-9
                || Math.abs(AHL_SOURCE_POSITION.x - 800.0) < 1e-9;
        if (!allowedAhlX
                || Math.abs(AHL_SOURCE_POSITION.y - 250.0) > 1e-9
                || Math.abs(AHL_SOURCE_POSITION.z - 5.0) > 1e-9) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 AHL must be (200|500|800, 250, 5), printed (%.4f, %.4f, %.4f)",
                    AHL_SOURCE_POSITION.x, AHL_SOURCE_POSITION.y, AHL_SOURCE_POSITION.z));
        }
        if (Math.abs(ACID_SOURCE_POSITION.x - 300.0) > 1e-9
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 acid must be (300, 375, 5), printed (%.4f, %.4f, %.4f)",
    ...
    static void assertFlowCourant() {
        if (FLOW_SPEED < 0) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 FLOW_SPEED must be >= 0, printed %.6f", FLOW_SPEED));
        }
        ...
        if (CHEMICAL_COURANT >= 1.0) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 chemical Courant=%.6f >= 1 (FLOW_SPEED=%.6f dt=%.6f dx=%.6f). Do not lower dt.",
                    CHEMICAL_COURANT, FLOW_SPEED, DT, FIELD_DX));
        }
        double[] allowed = {0.0, 0.25, 0.5, 0.666667, 1.0};
        boolean flowOk = false;
        for (double speed : allowed) {
            if (Math.abs(FLOW_SPEED - speed) < 1e-6) flowOk = true;
        }
        if (Math.abs(FLOW_SPEED - 8.0) < 1e-6) {
            throw new IllegalStateException(
                    "STOP SweepE03 do not rerun 8 um/s; cite SweepS5 seed 111 occupancy DEAD");
        }
        if (Math.abs(FLOW_SPEED - 2.0) < 1e-6 || Math.abs(FLOW_SPEED - 3.333333) < 1e-6) {
            throw new IllegalStateException(
                    "STOP SweepE03 2.0 and 3.333 um/s remain min-C excluded");
        }
        if (!flowOk) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepE03 FLOW_SPEED=%.6f is not in the frozen scout table", FLOW_SPEED));
        }
        CHEMICAL_BOUNDARY = FLOW_SPEED > 0 ? "OUTFLOW" : "NO_FLUX";
        if (FLOW_SPEED > 0 && !"OUTFLOW".equals(CHEMICAL_BOUNDARY)) {
            throw new IllegalStateException("STOP SweepE03 positive flow must be OUTFLOW");
        }
        if (FLOW_SPEED == 0 && !"NO_FLUX".equals(CHEMICAL_BOUNDARY)) {
            throw new IllegalStateException("STOP SweepE03 zero flow must be NO_FLUX");
        }
    }
    ...
    static void printGeometryBanner() {
        System.out.printf(Locale.US,
                "SweepE03 bound.x=%.6f bound.y=%.6f bound.z=%.6f%n", BOUND_X, BOUND_Y, BOUND_Z);
        System.out.printf(Locale.US,
                "SweepE03 grid.x=%d grid.y=%d grid.z=%d dx=%.6f dy=%.6f CFL_dt_max=%.6f dt=%.6f%n",
                GRID_X, GRID_Y, GRID_Z, FIELD_DX, FIELD_DY, DT_CFL_MAX, DT);
        System.out.printf(Locale.US,
                "SweepE03 field.ahl.source.x=%.6f field.ahl.source.y=%.6f field.ahl.source.z=%.6f rate=%.0f%n",
                AHL_SOURCE_POSITION.x, AHL_SOURCE_POSITION.y, AHL_SOURCE_POSITION.z, AHL_SOURCE_RATE);
        System.out.printf(Locale.US,
                "SweepE03 field.acid.source.x=%.6f field.acid.source.y=%.6f field.acid.source.z=%.6f rate=%.6g%n",
                ACID_SOURCE_POSITION.x, ACID_SOURCE_POSITION.y, ACID_SOURCE_POSITION.z, ACID_SOURCE_RATE);
        System.out.printf(Locale.US,
                "SweepE03 field.att0.x=%.6f field.att0.y=%.6f field.att0.z=%.6f%n",
                ATT0_POSITION.x, ATT0_POSITION.y, ATT0_POSITION.z);
        System.out.printf(Locale.US,
                "SweepE03 field.att2.x=%.6f field.att2.y=%.6f field.att2.z=%.6f%n",
                ATT2_POSITION.x, ATT2_POSITION.y, ATT2_POSITION.z);
        System.out.printf(Locale.US,
                "SweepE03 initial.pop=%d carrying.capacity=%d luminescence.alpha=%.12g luminescence.delta=%.12g%n",
                INITIAL_POP, CARRYING_CAPACITY, ALPHA_LUX, DELTA_LUX);
        System.out.printf(Locale.US,
                "SweepE03 FLOW_SPEED=%.6f transit_s=%s chemical_Courant=%.6f CFL_dt_max_flow=%s chemical_boundary=%s%n",
                FLOW_SPEED, formatTransit(TRANSIT_S), CHEMICAL_COURANT, formatTransit(DT_CFL_MAX_FLOW),
                CHEMICAL_BOUNDARY);
        if (FLOW_SPEED > 0) {
            System.out.println("SweepE03 advect=AHL+acid+att+rep Stokes=bacterium+BrownianParticle");
        } else {
            System.out.println("SweepE03 advect=off Stokes=bacterium+BrownianParticle (FLOW_SPEED=0)");
        }
    }
    ...
    static void writeSourcePositions(File directory) throws IOException {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write(String.format(Locale.US, "flow.speed.um_s=%.6f%n", FLOW_SPEED));
            ...
            w.write(FLOW_SPEED > 0 ? "advect=ahl,acid,att,rep\n" : "advect=off\n");
            w.write("stokes=bacterium,BrownianParticle\n");
            w.write("note=SweepE03: layout-by-flow scout; claim box 1000x500x10; AHL rate 1.28e8; K=1.6; tau_L=1500; SweepS5 Stage 6 upwind when FLOW>0; no recycling; no 8 um/s rerun\n");
        }
        if (SMOKE_NOT_NARMA) {
            try (BufferedWriter w = new BufferedWriter(
                    new FileWriter(new File(directory, "SMOKE_NOT_NARMA.txt")))) {
                w.write("This directory is HybridDish smoke timing only. Do not score as NARMA.\n");
            }
        }
    }
    ...
    static void writeMatlabMeta(File directory) throws IOException {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            if (ARM == Arm.BROWNIAN) {
                ...
                w.write("biologyReadout=R=window-mean Receiver_R 20x10; L=window-mean Lum_Mean 20x10; Death=window-last Input_Driven_Death 4x2\n");
            }
            w.write("see=examples/HybridDish/matlab/load_voxels.m and ridge_closed.m\n");
        }
    }
    ...
    static void writeRunStatus(OutputOwner output) {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write(String.format(Locale.US,
                    "arm=%s%nseed=%d%nbound.x=%.6f%nbound.y=%.6f%nbound.z=%.6f%ngrid.x=%d%ngrid.y=%d%ngrid.z=%d%ndx=%.6f%ndy=%.6f%nCFL_dt_max=%.6f%nahl.x=%.6f%nahl.y=%.6f%nahl.z=%.6f%nahl.rate=%.6f%nacid.x=%.6f%nacid.y=%.6f%nacid.z=%.6f%natt0.x=%.6f%natt0.y=%.6f%natt0.z=%.6f%natt2.x=%.6f%natt2.y=%.6f%natt2.z=%.6f%ninitial.pop=%d%ncarrying.capacity=%d%nalpha=%.12g%ndelta=%.12g%nflow.speed.um_s=%.6f%ntransit_s=%s%nchemical_Courant=%.6f%nCFL_dt_max_flow=%s%nchemical_boundary=%s%nsummary_rows=%d%nsample_rows=%d%nvoxel_rows=%d%n",
                    ARM.name().toLowerCase(Locale.ROOT), RNG_SEED,
    ...
    static void loadConfig(String path) {
        ...
        SMOKE_NOT_NARMA = Boolean.parseBoolean(
                p.getProperty("sweep.smoke.not.narma", String.valueOf(SMOKE_NOT_NARMA)));
        FLOW_SPEED = getDouble(p, "flow.speed.um_s", FLOW_SPEED);
