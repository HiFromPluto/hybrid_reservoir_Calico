...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanE4AC.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanE4AC.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanE4AC.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanE4AC.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanE4AC.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanE4AC.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanE4AC.pHFromAcidMm(
                            BSimReservoirPlanE4AC.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanE4AC.ReservoirBacterium bacterium : bacteria) {
