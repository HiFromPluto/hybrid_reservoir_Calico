...
/**
 * E4.1 one-way A1 AHL transducer on the frozen Narma10b claim dish.
 * MODEL_STATUS = HYPOTHETICAL_DESIGN_ENVELOPE. Not a digital twin.
 * No vesicle store. No two-way. Kinetics, layout, and FLOW=0 are frozen.
 */
public final class BSimReservoirPlanE4AC {
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        public void close() {
            ...
            throw new RuntimeException("Cannot close E4AC output", e);
            }
        }
    }

    public static void main(String[] args) {
        String configPath = args.length > 0 ? args[0] : "sim_config_e4ac.properties";
        loadConfig(configPath);
        if (args.length > 1 && "preview".equals(args[1])) HEADLESS = false;
        resetStaticState();
        if (FLOW_SPEED != 0.0)
            throw new IllegalStateException("E4AC claim dish requires FLOW_SPEED=0");
        if (ARM != Arm.SILENT) {
        ...
        System.out.printf(Locale.US,
                "E4AC MODEL_STATUS=%s digital_twin=false dish=%.0fx%.0fx%.0f CENTER "
                        + "FLOW_SPEED=%.1f AHL_source=(500,250,5)%n",
                MODEL_STATUS, BOUND_X, BOUND_Y, BOUND_Z, FLOW_SPEED);
        System.out.printf(Locale.US,
                "E4AC A1 g0=%.6g n_AC=%.6g K_AC=%.6g Jmax_A1=%.12e g(0)=%.12f g(0.5)=%.12f "
                        + "payload_rel_err=%.3e arm=%s seed=%d num.windows=%d pulse=%.0f window=%.0f%n",
        ...
        try {
            ...
            throw new RuntimeException("Cannot open E4AC output", e);
        }
            ...
            public void tick() {
                ...
                if (warmup && step % stepsInWindow < stepsInPulse) {
                    ahlField.addQuantity(AC_POSITIONS[1],
                            AHL_SOURCE_RATE * acGate(WARMUP_AHL_INPUT) * DT);
                }
                if (!warmup && window < NUM_WINDOWS && stepInWindow < stepsInPulse) {
                    ...
                    ahlField.addQuantity(AC_POSITIONS[1],
                            AHL_SOURCE_RATE * acGate(ahlInputs[window]) * DT);
        ...
        System.out.printf(Locale.US,
                "E4AC complete arm=%s FLOW_SPEED=%.1f AHL_finite=%s AHL_nonneg=%s "
    ...
    static void writeFeatureContract(File directory) throws IOException {
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write("transducer=A1_static_Hill_gate\n");
    ...
    static void writeRunStatus(OutputOwner output) {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write(String.format(Locale.US,
                    "arm=%s%nseed=%d%nMODEL_STATUS=%s%nJmax_A1=%.12e%ng0=%.6g%nn_AC=%.6g%nK_AC=%.6g%n"
                            + "FLOW_SPEED=%.1f%nAHL_finite=%s%nAHL_nonneg=%s%n"
                            + "summary_rows=%d%nsample_rows=%d%nvoxel_rows=%d%n",
                    ARM.name().toLowerCase(Locale.ROOT), RNG_SEED, MODEL_STATUS,
                    AHL_SOURCE_RATE, AC_G0, AC_N, AC_K, FLOW_SPEED,
                    ahlFinite ? "true" : "false", ahlNonNegative ? "true" : "false",
