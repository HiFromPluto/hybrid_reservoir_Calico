...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanBenchA2.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanBenchA2.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanBenchA2.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanBenchA2.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanBenchA2.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanBenchA2.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanBenchA2.pHFromAcidMm(
                            BSimReservoirPlanBenchA2.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanBenchA2.ReservoirBacterium bacterium : bacteria) {
