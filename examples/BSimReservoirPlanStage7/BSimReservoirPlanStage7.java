package BSimReservoirPlanStage7;
...
/**
 * Spatial Plan Stage 7. Copies frozen Stage 6 biology and spreads the four
 * grounded sources so their plumes are not the same spatial mode.
 * Stage 4 remains FAIL. Stage 5 remains PASS. Stage 6 remains PASS.
 * Kinetics, NARMA, and the Brownian comparison are not retuned or reopened.
 */
public final class BSimReservoirPlanStage7 {
    enum Arm { SILENT, DRIVEN }
    ...
    static String OUTPUT_DIR = "results/stage7_driven_seed101";
    static String INPUT_ATT_A_FILE = "input_att_a_40.txt";
    static String INPUT_AHL_FILE = "input_ahl_40.txt";
    static String INPUT_ATT_B_FILE = "input_att_b_40.txt";
    static String INPUT_ACID_FILE = "input_acid_40.txt";
    static String RUN_LABEL = "stage7_driven";
    ...
    static double ATT_SOURCE_RATE = PROD_RATE;
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        OutputOwner(File directory) throws IOException {
            ...
            summary.write("Run_Label;Stochastic_Replicate;Arm;Window;"
                    + "AttA_Input;AHL_Input;AttB_Input;Acid_Input;"
            ...
            if (samples != null) samples.write("Window;Sample;TimeInWindow_s;"
                    + "Input_AttA;Input_AHL;Input_AttB;Input_Acid;Total_Count;"
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close Stage 7 output", e);
            }
        }
    }

    public static void main(String[] args) {
        String configPath = args.length > 0 ? args[0] : "sim_config_stage7.properties";
        loadConfig(configPath);
        if (args.length > 1 && "preview".equals(args[1])) HEADLESS = false;
        resetStaticState();
        experimentRng = new Random(RNG_SEED);
        printSourcePositions();
        final double[] attAInputs = readGradedSequence(INPUT_ATT_A_FILE, "AttA");
        final double[] ahlInputs = readGradedSequence(INPUT_AHL_FILE, "AHL");
        final double[] attBInputs = readGradedSequence(INPUT_ATT_B_FILE, "AttB");
        final double[] acidInputs = readGradedSequence(INPUT_ACID_FILE, "acid");
        if (attAInputs.length < NUM_WINDOWS || ahlInputs.length < NUM_WINDOWS
                || attBInputs.length < NUM_WINDOWS || acidInputs.length < NUM_WINDOWS)
            throw new IllegalArgumentException("an input sequence is shorter than num.windows="
                    + NUM_WINDOWS);
        ...
        try {
            ...
            throw new RuntimeException("Cannot open Stage 7 output", e);
        }
        ...
        sim.setTicker(new BSimTicker() {
            ...
            public void tick() {
                ...
                if (!warmup && window < NUM_WINDOWS && stepInWindow == 0) {
                    if (window > 0) {
                        finishWindow(output, window - 1, attAInputs[window - 1],
                                ahlInputs[window - 1], attBInputs[window - 1],
                ...
                if (!warmup && window < NUM_WINDOWS && stepInWindow < stepsInPulse) {
                    attractantField.addQuantity(ATT_A_POSITION,
                            ATT_SOURCE_RATE * attAInputs[window] * DT);
                    attractantField.addQuantity(ATT_B_POSITION,
                            ATT_SOURCE_RATE * attBInputs[window] * DT);
                    ahlField.addQuantity(AHL_POSITION,
                    ...
                    if (interval || last) {
                        ...
                        if (WRITE_SAMPLES || WRITE_VOXELS)
                            writeSample(output, window, stepInWindow * DT,
                                    attAInputs[window], ahlInputs[window],
                                    attBInputs[window], acidInputs[window],
        ...
        try {
            ...
            while (completed[0] < NUM_WINDOWS) {
                finishWindow(output, completed[0], attAInputs[completed[0]],
                        ahlInputs[completed[0]], attBInputs[completed[0]],
        ...
        System.out.printf(Locale.US,
                "Stage7 complete arm=%s summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
                ARM.name().toLowerCase(Locale.ROOT), output.summaryRows, NUM_WINDOWS,
                output.sampleRows, output.voxelRows);
    }

    static void printSourcePositions() {
        System.out.printf(Locale.US,
                "SOURCE_POSITIONS AttractantA=(%.0f,%.0f,%.0f) AHL=(%.0f,%.0f,%.0f) "
                        + "Acid=(%.0f,%.0f,%.0f) AttractantB=(%.0f,%.0f,%.0f) flow=%.1f%n",
                ATT_A_POSITION.x, ATT_A_POSITION.y, ATT_A_POSITION.z,
                AHL_POSITION.x, AHL_POSITION.y, AHL_POSITION.z,
                ACID_SOURCE_POSITION.x, ACID_SOURCE_POSITION.y, ACID_SOURCE_POSITION.z,
                ATT_B_POSITION.x, ATT_B_POSITION.y, ATT_B_POSITION.z, FLOW_SPEED);
    }
    ...
    static void finishWindow(OutputOwner output, int window,
                             double attAInput, double ahlInput, double attBInput, double acidInput,
        ...
        try {
            output.summary.write(String.format(Locale.US,
                    ...
                    accumulator.lumSum / observations,
                    n == 0 ? 0.0 : lum[0],
                    ...
                    accumulator.lumSum,
                    n == 0 ? 0.0 : mean(pH),
                    n == 0 ? 0.0 : quantile(pH, .1),
                    n == 0 ? 0.0 : quantile(pH, .5),
                    n == 0 ? 0.0 : quantile(pH, .9),
                    population,
    ...
    static void writeSample(OutputOwner output, int window, double offset,
        ...
        try {
            ...
            double meanL = bacteria.isEmpty() ? 0.0 : lumSum / bacteria.size();
            if (output.samples != null) {
                output.samples.write(String.format(Locale.US,
                        "%d;%d;%.2f;%.2f;%.2f;%.2f;%.2f;%d;%d;%d;%d;%d;%d;%.9g;%.9g%n",
                        window, sample, offset, attAInput, ahlInput, attBInput, acidInput,
                        bacteria.size(), births, deaths, clampDeaths, acidDeaths, oobDeaths,
                        meanL, lumSum));
                output.sampleRows++;
            }
            if (output.voxels != null) {
                output.voxels.write(String.format(Locale.US,
                        "%d;%d;%.2f;%.2f;%.2f;%.2f;%.2f", window, sample, offset,
                        attAInput, ahlInput, attBInput, acidInput));
    ...
    static void writeVoxelHeader(BufferedWriter w) throws IOException {
        StringBuilder h = new StringBuilder(
                "Window;Sample;TimeInWindow_s;Input_AttA;Input_AHL;Input_AttB;Input_Acid");
    ...
    static void writeFeatureContract(File directory) throws IOException {
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write("rank_features=Den (20x10); Receiver_R / Mean_q (20x10); Lum_Mean / Mean_L (20x10); Input_Driven_Death (4x2)\n");
            w.write("rank_feature_count=608\n");
            w.write("excluded=raw Att/AHL_uM/pH voxels, occupancy, Births, Total_Deaths, Clamp_Deaths, OOB, Population, Lum_Sum\n");
            w.write("narma=not a Stage 7 gate\n");
        }
    }
    ...
    static void writeSourcePositions(File directory) throws IOException {
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write(String.format(Locale.US,
                    "AttractantA=%.0f,%.0f,%.0f%nAHL=%.0f,%.0f,%.0f%nAcid=%.0f,%.0f,%.0f%nAttractantB=%.0f,%.0f,%.0f%nflow=%.1f%n",
                    ATT_A_POSITION.x, ATT_A_POSITION.y, ATT_A_POSITION.z,
                    AHL_POSITION.x, AHL_POSITION.y, AHL_POSITION.z,
                    ACID_SOURCE_POSITION.x, ACID_SOURCE_POSITION.y, ACID_SOURCE_POSITION.z,
                    ATT_B_POSITION.x, ATT_B_POSITION.y, ATT_B_POSITION.z, FLOW_SPEED));
    ...
    static void loadConfig(String path) {
        ...
        ATT_SOURCE_RATE = getDouble(p, "field.att.source.rate", ATT_SOURCE_RATE);
        ...
        INPUT_ATT_A_FILE = p.getProperty("input.att.a.file", INPUT_ATT_A_FILE);
        INPUT_AHL_FILE = p.getProperty("input.ahl.file", INPUT_AHL_FILE);
        INPUT_ATT_B_FILE = p.getProperty("input.att.b.file", INPUT_ATT_B_FILE);
        ...
        if ("silent".equals(armName)) ARM = Arm.SILENT;
        else if ("driven".equals(armName)) ARM = Arm.DRIVEN;
        else throw new IllegalArgumentException("arm must be silent or driven");
        if (ARM == Arm.SILENT) {
            AHL_SOURCE_RATE = 0.0;
