...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanStage7.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanStage7.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanStage7.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanStage7.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanStage7.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanStage7.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanStage7.pHFromAcidMm(
                            BSimReservoirPlanStage7.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanStage7.ReservoirBacterium bacterium : bacteria) {
