#!/usr/bin/env python3
"""Evaluate Track A scalar RC gates on the frozen Stage 6 dish.

Stage 6 NARMA-10 remains PASS and is not modified. Track B is not started.
The Stage 6 val-slice leak (lambda chosen on X[88:], which included test)
is closed: lambda is picked on rows 88..109 only.
"""
...
LORENZ_SHA = "3bc69bbe32f6f2cf6a8638c4e2775133b4d27026a4223cbe783c057c19232f79"
MG_SHA = "e06810ea70f61bbe4baf61939f93b1e1cae9d92d9eee8d757865c4935dbac780"
LORENZ_XMIN = -16.849584157354
LORENZ_XMAX = 14.975551014173
MG_XMIN = 0.411423008740
MG_XMAX = 1.295296067959
...
def recompute_mg():
    x = np.empty(1000 + 201, dtype=float)
    x[0] = 1.2
    for t in range(len(x) - 1):
        x_tau = x[t - 17] if t - 17 >= 0 else 1.2
        x[t + 1] = x[t] + 0.2 * x_tau / (1.0 + x_tau ** 10) - 0.1 * x[t]
    return x[1000:1000 + 201]
...
def load_target(path, task):
    ...
    if task == "lorenz":
        ...
        if not np.allclose(x, x_drive, rtol=0, atol=1e-9):
            raise ValueError("lorenz_target.csv x does not match RK4")
        if not np.allclose(y, y_next, rtol=0, atol=1e-9):
            raise ValueError("lorenz_target.csv y_next does not match RK4")
        if not np.allclose(u, u_hat, rtol=0, atol=1e-9):
            raise ValueError("lorenz_target.csv u does not match affine map")
        ...
        return u, y
    if task == "mg":
        y = np.array([float(row["x_next"]) for row in rows], dtype=float)
        recorded = recompute_mg()
        x_drive = recorded[:NUM_WINDOWS]
        x_next = recorded[1:NUM_WINDOWS + 1]
        xmin, xmax = float(np.min(x_drive)), float(np.max(x_drive))
        u_hat = affine_u(x_drive, xmin, xmax)
        if not np.allclose(x, x_drive, rtol=0, atol=1e-9):
            raise ValueError("mg_target.csv x does not match the discrete map")
        if not np.allclose(y, x_next, rtol=0, atol=1e-9):
            raise ValueError("mg_target.csv x_next does not match the discrete map")
        if not np.allclose(u, u_hat, rtol=0, atol=1e-9):
            raise ValueError("mg_target.csv u does not match affine map")
        if abs(xmin - MG_XMIN) > 1e-9 or abs(xmax - MG_XMAX) > 1e-9:
            raise ValueError("Mackey-Glass xmin/xmax drifted from PROTOCOL.md")
        if digest != MG_SHA:
            raise ValueError("Mackey-Glass u SHA-256 drifted from PROTOCOL.md")
        return u, y
...
def no_track_b(bencha_java):
    text = Path(bencha_java).read_text(encoding="utf-8")
...
def write_combined_markdown(path, sections):
    lines = [
        "# Track A gate evidence",
        "",
        "Track A copies frozen Stage 6 and tests Lorenz'63 then Mackey-Glass",
        "against a Brownian density null and a silent-source control. Stage 6",
        "NARMA-10 remains **PASS**. Mechanisms were not retuned. Track B was",
        "not started. The Stage 6 val-slice leak is closed.",
        "",
    ]
    for key, title in (("lorenz", "Lorenz'63"), ("mg", "Mackey-Glass")):
        evidence = sections.get(key)
        ...
        lines.extend([
            ...
            f"| 5. Stage 6 NARMA-10 PASS; Stage 6 Java untouched; no glucose/Danino/extra ACs/Track B | **{'PASS' if gates['prior_stage_labels_pass'] else 'FAIL'}** |",
            "",
            "### Test NRMSE",
            "",
    ...
    lines.extend([
        "## Protocol",
        "",
        "See `PROTOCOL.md`. Affine `u ∈ [0, 0.5]`; acid held at 0.5; washout 40 /",
        "train 110 / test 50; lambda selected on windows 128..149 only.",
        "",
...
def default_run_dirs(task, arm):
    return [str(HERE / "results" / f"{task}_{arm}_seed{seed}") for seed in SEEDS]


def evaluate_task(args):
    task = args.task
    target_name = "lorenz_target.csv" if task == "lorenz" else "mg_target.csv"
    target_path = Path(args.target) if args.target else HERE / target_name
    u, y = load_target(target_path, task)
    brownian = args.brownian or default_run_dirs(task, "brownian")
    silent = args.silent or default_run_dirs(task, "silent")
    driven = args.driven or default_run_dirs(task, "driven")
    grouped = {
    ...
    prior_ok = (
        ...
        and no_track_b(HERE / "BSimReservoirPlanBenchA.java")
    )
    ...
    if not gates["driven_beats_brownian_pass"]:
        ...
        failure_reason = "provenance: Stage 6 GATE_EVIDENCE is not PASS, Stage 6 Java was touched, or Track B / glucose / Danino / extra ACs leaked in."
    else:
        failure_reason = ""
    ...
    evidence = {
        "schema": "BSimReservoirPlanBenchA-gates-v1",
        "task": task,
...
def main():
    ...
    parser.add_argument("--task", choices=("lorenz", "mg"), required=True)
    ...
    evidence_path = Path(args.evidence) if args.evidence else (
        HERE / "results" / f"{args.task}_gate_evidence.json"
    )
    ...
    sections = {}
    for key in ("lorenz", "mg"):
        path = HERE / "results" / f"{key}_gate_evidence.json"
        if path.exists():
            sections[key] = json.loads(path.read_text(encoding="utf-8"))
    sections[args.task] = evidence
    markdown_path = Path(args.markdown)
    markdown_path.parent.mkdir(parents=True, exist_ok=True)
    write_combined_markdown(markdown_path, sections)
    ...
    print(f"{args.task} Overall: {'PASS' if evidence['overall_gate_pass'] else 'FAIL'}")
