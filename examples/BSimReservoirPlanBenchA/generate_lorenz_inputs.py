#!/usr/bin/env python3
"""Generate the frozen Track A Lorenz'63 AHL sequence and target sidecar.

Deterministic ODE. Do not retune sigma/rho/beta, dt, IC, or transient after
seeing NRMSE. Acid hold is copied from Stage 6, not regenerated.
"""
...
def main() -> None:
    ...
    ahl_path = directory / "input_ahl_lorenz200.txt"
    acid_path = directory / "input_acid_held05_200.txt"
    target_path = directory / "lorenz_target.csv"

    ahl_path.write_text(
        "# Frozen Track A Lorenz'63 AHL input. Affine x[0..199] onto [0, 0.5].\n"
        "# sigma=10, rho=28, beta=8/3, RK4 dt=0.02, IC=(1,1,1), discard 5000 steps.\n"
        "# Do not regenerate after seeing NRMSE.\n"
    ...
    print(f"dt_L={DT_L} ic={IC} transient_steps={TRANSIENT_STEPS}")
