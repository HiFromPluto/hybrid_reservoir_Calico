...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanBenchA.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanBenchA.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanBenchA.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanBenchA.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanBenchA.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanBenchA.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanBenchA.pHFromAcidMm(
                            BSimReservoirPlanBenchA.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanBenchA.ReservoirBacterium bacterium : bacteria) {
