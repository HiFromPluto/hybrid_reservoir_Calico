...
/**
 * E5.2 occupancy-first one-AHL CRP scout (Task B only). Encoding selected on occupancy, then frozen. Claim dish
 * copied from Narma10b. Mechanisms are not retuned. Track B, Task A,
 * and sequential batching are not started.
 */
public final class BSimReservoirPlanE52CRP {
    ...
    static int NUM_WINDOWS = 48, INITIAL_POP = 1800, CARRYING_CAPACITY = 2000;
    static boolean HEADLESS = true, WRITE_SAMPLES = true, WRITE_VOXELS = true;
    static long RNG_SEED = 111;
    static Arm ARM = Arm.DRIVEN;
    static String OUTPUT_DIR = "results/e52_driven_p0001_seed111";
    static String INPUT_AHL_FILE = "input/input_ahl_p0001.txt";
    static String INPUT_ACID_FILE = "input/input_acid_held05_48.txt";
    static String RUN_LABEL = "e52_driven";
    static String STOCHASTIC_REPLICATE = "driven_p0001_seed111";
    static int PATIENT_ID = -1;
    static String ENCODING_MAP_ID = "UNSET";
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close Narma10b output", e);
            }
        }
    }

    public static void main(String[] args) {
        String configPath = args.length > 0 ? args[0] : "sim_config_e52.properties";
        ...
        try {
            ...
            throw new RuntimeException("Cannot open Narma10b output", e);
        }
        ...
        originalOut.printf(Locale.US,
                "E5.2 map=%s CENTER FLOW=0 Jmax=1.28e8%n"
                        + "patient=%d arm=%s seed=%d u[0]=%.12f warmup.ahl.input=%.12f "
                        + "num.windows=%d ahl_site=(500,250,5)%n",
                ENCODING_MAP_ID, PATIENT_ID, ARM.name().toLowerCase(Locale.ROOT), RNG_SEED,
                ahlInputs[0], WARMUP_AHL_INPUT, NUM_WINDOWS);
        try {
            if (HEADLESS) {
        ...
        System.out.printf(Locale.US,
                "E5.2 complete arm=%s map=%s patient=%d summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
                ARM.name().toLowerCase(Locale.ROOT), ENCODING_MAP_ID, PATIENT_ID, output.summaryRows, NUM_WINDOWS,
    ...
    static void writeFeatureContract(File directory) throws IOException {
        try (BufferedWriter w = new BufferedWriter(
            ...
            if (ARM == Arm.BROWNIAN) {
                ...
                w.write("note=AHL source is off; acid may still be held; particles do not sense; Den only\n");
            } else {
    ...
    static void loadConfig(String path) {
        ...
        PATIENT_ID = getInt(p, "patient.id", PATIENT_ID);
        ENCODING_MAP_ID = p.getProperty("encoding.map.id", ENCODING_MAP_ID).trim();
    }
