...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanWaveformS.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanWaveformS.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanWaveformS.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanWaveformS.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanWaveformS.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanWaveformS.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanWaveformS.pHFromAcidMm(
                            BSimReservoirPlanWaveformS.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanWaveformS.ReservoirBacterium bacterium : bacteria) {
