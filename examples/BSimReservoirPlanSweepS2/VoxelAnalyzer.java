...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanSweepS2.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanSweepS2.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanSweepS2.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanSweepS2.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanSweepS2.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanSweepS2.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanSweepS2.pHFromAcidMm(
                            BSimReservoirPlanSweepS2.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanSweepS2.ReservoirBacterium bacterium : bacteria) {
