package BSimReservoirPlanSweepS2;
...
/**
 * Sweep Step 2: AHL source-rate occupancy scout on the frozen claim dish.
 * Position stays (500, 250, 5). Only field.ahl.source.rate may change.
 * K stays 1.6. Acid stays (300, 375, 5). Attractants stay silent.
 */
public final class BSimReservoirPlanSweepS2 {
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close SweepS2 output", e);
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        System.out.printf(Locale.US,
                "SweepS2 AHL source=(%.4f, %.4f, %.4f) rate=%.0f acid source=(%.4f, %.4f, %.4f)%n",
        ...
        try {
            ...
            throw new RuntimeException("Cannot open SweepS2 output", e);
        }
        ...
        System.out.printf(Locale.US,
                "SweepS2 complete arm=%s ahl=(%.1f,%.1f,%.1f) rate=%.0f summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
    ...
    static void assertClaimAhlPosition() {
        if (Math.abs(AHL_SOURCE_POSITION.x - 500.0) > 1e-9
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepS2 AHL must be (500, 250, 5), printed (%.4f, %.4f, %.4f)",
                    AHL_SOURCE_POSITION.x, AHL_SOURCE_POSITION.y, AHL_SOURCE_POSITION.z));
        }
    }
    ...
    static void writeSourcePositions(File directory) throws IOException {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write("note=SweepS2: AHL position locked at (500,250,5); only field.ahl.source.rate changes; K=1.6; acid not moved\n");
        }
    }
    ...
    static void writeRunStatus(OutputOwner output) {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write(String.format(Locale.US,
                    "arm=%s%nseed=%d%nahl.x=%.6f%nahl.y=%.6f%nahl.z=%.6f%nahl.rate=%.6f%nsummary_rows=%d%nsample_rows=%d%nvoxel_rows=%d%n",
