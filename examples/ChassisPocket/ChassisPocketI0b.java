...
/**
 * Job I0b — DoorWeir at T4/T5 width W=20 µm.
 *
 * <p>Every parameter is frozen in PROTOCOL.md before this file existed.
 * Chamber 100x100x1 um, +y gap W=20 centred at x=50, founder at (50,80),
 * uniform Warren bath (no PDE), SYMMETRY_BROKEN seed 101, Job 3 Hertzian.
 *
 ...
 */
public final class ChassisPocketI0b {

    private ChassisPocketI0b() {}

    static final String EXPORT_DIR = "results/i0b_seed101/";
    ...
    static void run(boolean smoke) {
        ...
        System.out.println("ChassisPocket I0b -> " + EXPORT_DIR + CSV_NAME);
        ...
        spillCum[0] += spillSinceLog[0];
        progress.println("I0b finished  t=" + (int) sim.getTime() + "  N=" + cells.size()
        ...
        if (filament[0]) {
            throw new IllegalStateException("I0b filament at N=" + cells.size());
        }
        if (smoke) {
            Vector3d com = centreOfMass(cells);
            double R = colonyRadius(cells, com);
            ...
            if (off > 0.05 || rr > FILAMENT_R_RDISC) {
                throw new IllegalStateException("I0b smoke FAIL  offplane=" + off
                        + "  R/R_disc=" + rr);
            }
            progress.printf(Locale.US,
                    "I0b smoke OK  N=%d  offplane=%.3f  R/R_disc=%.2f  spill_cum=%d  y_max=%.2f%n",
