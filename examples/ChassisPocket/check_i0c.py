#!/usr/bin/env python3
"""Evaluate ChassisPocket I0c DoorWeir gates (W=20 µm, t_end=25950 s).
...
CSV_PATH = ROOT / "results" / "i0c_seed101" / "colony_timeseries.csv"
...
def gate4_fill(rows):
    ...
    if not any(int(r["door_contact"]) == 1 for r in rows):
        errs.append(
            "door_contact never fired: I0c did not ask the door question "
...
def main() -> int:
    ...
    print("Gate I0c.6 honesty: scored by standing memo (AHL/Hill/NARMA OFF, W=20, founder y=80)")
    print()
    last = rows[-1]
    ...
    print()
    if failures:
        print("I0c OVERALL: FAIL")
        return 1
    print("I0c OVERALL: PASS")
    return 0
