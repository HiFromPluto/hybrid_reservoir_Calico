...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanSweepS1.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanSweepS1.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanSweepS1.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanSweepS1.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanSweepS1.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanSweepS1.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanSweepS1.pHFromAcidMm(
                            BSimReservoirPlanSweepS1.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanSweepS1.ReservoirBacterium bacterium : bacteria) {
