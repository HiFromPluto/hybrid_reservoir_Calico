...
/**
 * PocketMonolayer-T5 Danino-class height weir. Clone of T4 chip (wall-mask,
 * bus advection, cyan AC visual). Growth on; clamp death off; particle
 * z weir 2 um; Z2_REP labelled xy overlap spring. Chemical box stays
 * 10 um. No NARMA, no LuxI, no J_max retune, no k_ov retune.
 */
public final class BSimPocketMonolayerT5 {
    enum Arm { Z2_NOREP, Z2_REP }
    ...
    /** ENGINEERING, Fluigi/Danino ~1.65 µm class, rounded. Particle constraint only. */
    static final double H_WEIR_FROZEN = 2.0;
    /** ENGINEERING, not fitted. Do not retune after seeing N. */
    static final double K_OV_FROZEN = 50.0;
    static final double FORCE_CAP_FROZEN = 100.0;
    ...
    static boolean REPULSION_ON = false;
    ...
    static Arm ARM = Arm.Z2_NOREP;
    static String OUTPUT_DIR = "results/runs/Z2_NOREP";
    static String RUN_LABEL = "z2_norep";
    static double ACID_CELL_PRODUCTION_RATE = 0.0;
    static double H_WEIR = H_WEIR_FROZEN;
    static double K_OV = K_OV_FROZEN;
    static double FORCE_CAP = FORCE_CAP_FROZEN;
    ...
    public static final class ReservoirBacterium extends BSimBacterium {
        ...
        void clampZWeir() {
            double r = getRadius();
            if (2.0 * r > H_WEIR) {
                position.z = 0.5 * H_WEIR;
            } else {
                double lo = r;
                double hi = H_WEIR - r;
                position.z = Math.max(lo, Math.min(hi, position.z));
            }
        }
        ...
        public void action() {
            super.action();
            if (!GROWTH_ON)
                throw new IllegalStateException("T5 growth must stay on");
            if (ACID_CELL_PRODUCTION_RATE != 0.0)
                throw new IllegalStateException("T5 acid cell production must stay off");
            if (!CLAMP_OFF)
                throw new IllegalStateException("T5 clamp death must stay off");
         ...
         * solid except the door (necked: W=20 mouth). Do not pull roof-hits
         * into the neck. Do not HybridDish-mirror the outer 400 µm box.
         */
    ...
    /**
     * Z2_REP only. Predeclared soft overlap spring in xy.
     * F = k_ov (2R - d) r_hat, cap 100 pN. Not fitted.
     */
    static void applyOverlapSprings() {
        int n = bacteria.size();
        Vector3d f = new Vector3d();
        for (int i = 0; i < n; i++) {
            ReservoirBacterium a = bacteria.get(i);
            for (int j = i + 1; j < n; j++) {
                ReservoirBacterium b = bacteria.get(j);
                Vector3d pa = a.getPosition();
                Vector3d pb = b.getPosition();
                double dx = pa.x - pb.x;
                double dy = pa.y - pb.y;
                double d = Math.hypot(dx, dy);
                double twoR = a.getRadius() + b.getRadius();
                if (d >= twoR) continue;
                double mag = K_OV * (twoR - d);
                if (mag > FORCE_CAP) mag = FORCE_CAP;
                double hx;
                double hy;
                if (d < 1e-12) {
                    hx = 1.0;
    ...
    public static void main(String[] args) {
        ...
        if (Math.abs(DT - DT_FROZEN) > 1e-12)
            throw new IllegalArgumentException("T5 freezes dt=0.02; got " + DT);
        if (!SMOKE && Math.abs(DURATION - DURATION_FROZEN) > 1e-12)
            throw new IllegalArgumentException("T5 freezes duration=18000 s; got " + DURATION);
        if (!GROWTH_ON)
            throw new IllegalArgumentException("T5 growth.on must be true");
        if (!CLAMP_OFF)
            throw new IllegalArgumentException("T5 clamp.off must be true");
        if (CARRYING_CAPACITY != CARRYING_CAPACITY_FROZEN)
            throw new IllegalArgumentException("T5 carrying capacity must be 4000");
        if (Math.abs(H_WEIR - H_WEIR_FROZEN) > 1e-12)
            throw new IllegalArgumentException("T5 weir.h.um must stay 2.0; got " + H_WEIR);
        if (Math.abs(K_OV - K_OV_FROZEN) > 1e-12)
            throw new IllegalArgumentException("T5 k.ov must stay 50; got " + K_OV);
        if (Math.abs(FORCE_CAP - FORCE_CAP_FROZEN) > 1e-12)
            throw new IllegalArgumentException("T5 force.cap must stay 100; got " + FORCE_CAP);
        if (ARM == Arm.Z2_REP && !REPULSION_ON)
            throw new IllegalArgumentException("Z2_REP requires repulsion.on=true");
        if (ARM == Arm.Z2_NOREP && REPULSION_ON)
            throw new IllegalArgumentException("Z2_NOREP requires repulsion.on=false");
        if (RNG_SEED != 101)
            throw new IllegalArgumentException("T5 rng.seed must be 101");
        if (INITIAL_POP != 50)
            throw new IllegalArgumentException("T5 initial.pop must be 50");
        if (ACID_CELL_PRODUCTION_RATE != 0.0)
            throw new IllegalArgumentException("T5 acid cell production must be 0");
        ...
        System.err.printf(Locale.US,
                "WEIR h=%.1f um  repulsion=%s  k_ov=%.1f pN/um  clamp.death=off%n",
                H_WEIR, Boolean.toString(REPULSION_ON), K_OV);
        ...
        if (CELLS) {
            for (int i = 0; i < INITIAL_POP; i++) {
                ...
                ReservoirBacterium b = new ReservoirBacterium(
                        sim, new Vector3d(x, y, 0.5 * H_WEIR), ahlField);
                b.setRadius();
        ...
        sim.setTicker(new BSimTicker() {
            ...
            public void tick() {
                ...
                if (CELLS) {
                    ...
                    int room = CARRYING_CAPACITY - bacteria.size();
                    if (room < 0) room = 0;
                    int keep = Math.min(children.size(), room);
                    for (int i = 0; i < keep; i++)
                        bacteria.add(children.get(i));
                    births[0] += keep;
                    children.clear();
        ...
        System.out.printf(Locale.US, "PocketMonolayerT5 done arm=%s dir=%s%n", ARM.name(), OUTPUT_DIR);
    }
    ...
    static void writeSummary(ChipField field, int nStart, int nMax, int spillover,
        ...
        try (BufferedWriter w = new BufferedWriter(new FileWriter(new File(dir, "summary.csv")))) {
            w.write("Arm,Drive,Model,Label,Smoke,dx_um,dt_s,W_um,L_n_um,t_end_s,u_on,"
                    ...
                    + "ClampOff,Weir_um,Repulsion,K_ov_pN_per_um,"
                    + "CellMeanR,CellMeanL,CellsWriteAHL,C_SW,C_SE,C_NW,C_NE,"
            ...
            w.write(String.format(Locale.US,
                    "%s,STEP_ON,bsim_java_chip,POPULATION,%s,%.1f,%.4f,%.1f,%.1f,%.1f,%.2f,"
                            ...
                            + "%.9g,%s,%s,%d,%d,%d,%d,%d,%d,%s,%d,%s,%.1f,%s,%.1f,%.9g,%.9g,%s,"
                            + "%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%.9g,%s,%d,%d,%d%n",
                    ...
                    "true", CARRYING_CAPACITY, Boolean.toString(CLAMP_OFF), H_WEIR,
                    Boolean.toString(REPULSION_ON), K_OV,
                    cellR, cellL, "false",
        ...
        System.err.printf(Locale.US,
                "%s occupancy=%s population=%s mean_R=%.6f C=%.4f uM residual/dom=%.3g "
                        + "N=%d->%d Nmax=%d spill=%d births=%d clamp=%d repulsion=%s%n",
                ARM.name(), occ, pop, pocketR, pocketC, vsDom, nStart, bacteria.size(),
                nMax, spillover, births, clampRemovals, Boolean.toString(REPULSION_ON));
    }
    ...
    static void printTheory() {
        ...
        System.out.println("PocketMonolayer-T5 THEORY (z weir + labelled overlap; T3 chip)");
        System.out.printf(Locale.US, "  J_max=%.3e molecules/s  V=%.0f um^3 (AHL box, not weir)%n", J_MAX, v);
        ...
        System.out.printf(Locale.US,
                "  h_weir=%.1f um  k_ov=%.1f pN/um  force_cap=%.0f pN%n",
                H_WEIR_FROZEN, K_OV_FROZEN, FORCE_CAP_FROZEN);
        System.out.println("  Hypothesis: occupancy ALIVE on both arms (T3 class).");
        System.out.println("  Z2_NOREP expect EMPTY (height-only). Z2_REP unknown.");
        System.out.println("  Do not retune K, J_max, growth, or k_ov after seeing N.");
        System.out.println("  LIVING_DECISION=STOP_AFTER_MONOLAYER_SCREEN");
    }
    ...
    static void loadConfig(String path) {
        ...
        H_WEIR = getDouble(p, "weir.h.um", H_WEIR);
        K_OV = getDouble(p, "k.ov.pN.per.um", K_OV);
        FORCE_CAP = getDouble(p, "force.cap.pN", FORCE_CAP);
        String armName = p.getProperty("arm", ARM.name()).trim().toUpperCase(Locale.ROOT);
        ARM = Arm.valueOf(armName);
        REPULSION_ON = Boolean.parseBoolean(p.getProperty("repulsion.on",
                ARM == Arm.Z2_REP ? "true" : "false"));
        CELLS = Boolean.parseBoolean(p.getProperty("cells", "true"));
        if (!CELLS)
            throw new IllegalArgumentException("T5 arms are living; cells must be true");
        ...
        if (Math.abs(growthRateCfg - GROWTH_RATE) > 1e-12)
            throw new IllegalArgumentException("T5 growth.rate must stay 4*pi/1800; got " + growthRateCfg);
    }
