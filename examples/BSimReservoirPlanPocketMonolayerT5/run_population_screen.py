#!/usr/bin/env python3
"""Run the two predeclared PocketMonolayer-T5 population arms.
...
ARM_CONFIGS = (
    ("Z2_NOREP", "config/z2_norep.properties"),
    ("Z2_REP", "config/z2_rep.properties"),
)
...
def refuse_narma() -> None:
    ...
    for token in FORBIDDEN:
        if token in blob:
            raise SystemExit("PocketMonolayer-T5 runner cannot see a NARMA target.")


def compile_job() -> None:
    cmd = ["cmd", "/c", "compile_and_run.cmd"]
    print("Compiling PocketMonolayerT5...")
...
def main() -> int:
    ...
    for arm, config in ARM_CONFIGS:
        if args.primary_only and arm != "Z2_NOREP":
            continue
        if not started:
        ...
        if arm == "Z2_NOREP" and not args.smoke:
            occ = occupancy_of(arm)
            if occ == "DEAD":
                print("Z2_NOREP occupancy DEAD. Stop. Do not raise J_max.")
                check = subprocess.run(
                    [sys.executable, str(HERE / "check_pocketmonolayer_t5.py")],
                    cwd=HERE,
                )
