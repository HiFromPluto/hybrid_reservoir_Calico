package BSimReservoirPlanE51CRP;
...
/**
 * E5.1 isolated one-AHL horizon-1 CRP scout (Task B only). Claim dish
 * copied from Narma10b. Mechanisms are not retuned. Track B, Task A,
 * and sequential batching are not started.
 */
public final class BSimReservoirPlanE51CRP {
    ...
    public static void main(String[] args) {
        String configPath = args.length > 0 ? args[0] : "sim_config_e51.properties";
        ...
        originalOut.printf(Locale.US,
                "E5.1 map=one-AHL_CRP u[n]=0.5*Xnorm[p,n,0] CENTER FLOW=0%n"
                        + "patient=%d arm=%s seed=%d u[0]=%.12f warmup.ahl.input=%.12f "
                        + "num.windows=%d Jmax=%.4e ahl_site=(500,250,5)%n",
                PATIENT_ID, ARM.name().toLowerCase(Locale.ROOT), RNG_SEED,
                ahlInputs[0], WARMUP_AHL_INPUT, NUM_WINDOWS, AHL_SOURCE_RATE);
        try {
            if (HEADLESS) {
        ...
        System.out.printf(Locale.US,
                "E5.1 complete arm=%s patient=%d summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
                ARM.name().toLowerCase(Locale.ROOT), PATIENT_ID, output.summaryRows, NUM_WINDOWS,
