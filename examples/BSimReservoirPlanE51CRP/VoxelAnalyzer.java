...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanE51CRP.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanE51CRP.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanE51CRP.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanE51CRP.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanE51CRP.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanE51CRP.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanE51CRP.pHFromAcidMm(
                            BSimReservoirPlanE51CRP.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanE51CRP.ReservoirBacterium bacterium : bacteria) {
