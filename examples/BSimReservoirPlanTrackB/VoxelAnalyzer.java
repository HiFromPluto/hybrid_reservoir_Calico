...
public final class VoxelAnalyzer {
    public static final class VoxelReadout {
        public final double[] ahlUm, pH, attConc, repConc, meanResponse, fractionQAboveHalf;
        public final double[] meanLum, sumLum;
        public final int[] density, births, clampDeaths, acidDeaths, oobDeaths;

        VoxelReadout(double[] ahlUm, double[] pH, double[] attConc, double[] repConc,
                     int[] density, double[] meanResponse, double[] fractionQAboveHalf,
                     double[] meanLum, double[] sumLum, int[] births,
                     int[] clampDeaths, int[] acidDeaths, int[] oobDeaths) {
            this.ahlUm = ahlUm;
            this.pH = pH;
            this.attConc = attConc;
            this.repConc = repConc;
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanTrackB.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanTrackB.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanTrackB.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanTrackB.DeathCause.OOB) oobDeathAccum[bin]++;
    }
    ...
    public VoxelReadout analyze(BSimChemicalField ahlField,
                                ...
                                List<BSimReservoirPlanTrackB.ReservoirBacterium> bacteria) {
        int total = getStateVoxels();
        double[] ahlUm = new double[total], pH = new double[total];
        double[] attConc = new double[total], repConc = new double[total];
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanTrackB.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanTrackB.pHFromAcidMm(
                            BSimReservoirPlanTrackB.acidMmFromConc(acidField.getConc(centre)));
                    attConc[id] = attractantField.getConc(centre);
                    repConc[id] = repellentField.getConc(centre);
                }
            }
        }

        for (BSimReservoirPlanTrackB.ReservoirBacterium bacterium : bacteria) {
        ...
        return new VoxelReadout(ahlUm, pH, attConc, repConc, density, meanResponse,
                fractionQAboveHalf, meanLum, lumSum, birthAccum.clone(),
                clampDeathAccum.clone(), acidDeathAccum.clone(), oobDeathAccum.clone());
    }
