package BSimReservoirPlanTrackB;
...
/**
 * Track B: five one-way ACs and a synthetic 5-channel patient task on a
 * copy of the frozen Stage 6 dish. Kinetics are not retuned. Stage 6
 * NARMA-10 remains PASS. Stage 9 product-bit FAIL and BenchA Lorenz FAIL
 * are not rewritten.
 */
public final class BSimReservoirPlanTrackB {
    enum Arm { BROWNIAN, SILENT, DRIVEN, SINGLESITE }
    ...
    static String OUTPUT_DIR = "results/trackb_driven_seed101";
    static String INPUT_AHL_FILE = "input_ahl_trackb200.txt";
    static String INPUT_ACID_FILE = "input_acid_trackb200.txt";
    static String INPUT_ATTA_FILE = "input_atta_trackb200.txt";
    static String INPUT_ATTB_FILE = "input_attb_trackb200.txt";
    static String INPUT_REP_FILE = "input_rep_trackb200.txt";
    static String RUN_LABEL = "trackb_driven";
    ...
    static final double PROD_RATE = 1e6; // Engineering; do not raise after AUC.
    ...
    /** Frozen Track B sites. Do not inject at Stage 6 silent (250,250,5) or (750,250,5). */
    static final Vector3d AHL_POSITION = new Vector3d(500, 250, 5);
    static final Vector3d ACID_SOURCE_POSITION = new Vector3d(300, 375, 5);
    static final Vector3d ATT_A_POSITION = new Vector3d(150, 100, 5);
    static final Vector3d ATT_B_POSITION = new Vector3d(850, 400, 5);
    static final Vector3d REP_POSITION = new Vector3d(150, 400, 5);
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        OutputOwner(File directory) throws IOException {
            ...
            summary.write("Run_Label;Stochastic_Replicate;Arm;Window;AHL_Input;Acid_Input;"
                    + "AttA_Input;AttB_Input;Rep_Input;"
            ...
            if (samples != null) samples.write("Window;Sample;TimeInWindow_s;"
                    + "Input_AHL;Input_Acid;Input_AttA;Input_AttB;Input_Rep;Total_Count;"
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close Track B output", e);
            }
        }
    }

    public static void main(String[] args) {
        String configPath = args.length > 0 ? args[0] : "sim_config_trackb.properties";
        ...
        final double[] attAInputs = readGradedSequence(INPUT_ATTA_FILE, "AttA");
        final double[] attBInputs = readGradedSequence(INPUT_ATTB_FILE, "AttB");
        final double[] repInputs = readGradedSequence(INPUT_REP_FILE, "Rep");
        if (ahlInputs.length < NUM_WINDOWS || acidInputs.length < NUM_WINDOWS
                || attAInputs.length < NUM_WINDOWS || attBInputs.length < NUM_WINDOWS
                || repInputs.length < NUM_WINDOWS)
            throw new IllegalArgumentException("an input file is shorter than num.windows="
                    + NUM_WINDOWS);
        ...
        try {
            ...
            throw new RuntimeException("Cannot open Track B output", e);
        }
        ...
        sim.setTicker(new BSimTicker() {
            ...
            public void tick() {
                ...
                if (!warmup && window < NUM_WINDOWS && stepInWindow == 0) {
                    if (window > 0) {
                        finishWindow(output, window - 1,
                                ahlInputs[window - 1], acidInputs[window - 1],
                                attAInputs[window - 1], attBInputs[window - 1],
                                repInputs[window - 1], births[0], deaths[0], clampDeaths[0],
                ...
                if (warmup && step % stepsInWindow < stepsInPulse) {
                    ahlField.addQuantity(AHL_POSITION,
                ...
                if (!warmup && window < NUM_WINDOWS && stepInWindow < stepsInPulse) {
                    ...
                    if (ARM == Arm.DRIVEN || ARM == Arm.BROWNIAN) {
                        attractantField.addQuantity(ATT_A_POSITION,
                                PROD_RATE * attAInputs[window] * DT);
                        attractantField.addQuantity(ATT_B_POSITION,
                                PROD_RATE * attBInputs[window] * DT);
                        repellentField.addQuantity(REP_POSITION,
                                PROD_RATE * repInputs[window] * DT);
                    }
                }
                    ...
                    if (interval || last) {
                        ...
                        if (WRITE_SAMPLES || WRITE_VOXELS)
                            writeSample(output, window, stepInWindow * DT,
                                    ...
                                    attAInputs[window], attBInputs[window],
                                    repInputs[window], births[0], deaths[0], clampDeaths[0],
                                    acidDeaths[0], oobDeaths[0],
                                    ahlField, acidField, attractantField, repellentField);
                    }
                }
            }
        });
        ...
        sim.setDrawer(new BSimP3DDrawer(sim, 800, 600) {
            ...
            public void scene(PGraphics3D p3d) {
                ...
                draw(attractantField, Color.CYAN, (float) (255.0 / 50.0));
                draw(repellentField, Color.RED, (float) (255.0 / 50.0));
                if (ARM == Arm.BROWNIAN) {
                    for (BrownianParticle particle : particles)
        ...
        try {
            ...
            while (completed[0] < NUM_WINDOWS) {
                finishWindow(output, completed[0],
                        ahlInputs[completed[0]], acidInputs[completed[0]],
                        attAInputs[completed[0]], attBInputs[completed[0]],
                        repInputs[completed[0]], births[0], deaths[0], clampDeaths[0],
                        acidDeaths[0], oobDeaths[0], windowAccumulator);
                completed[0]++;
            }
        } finally {
        ...
        System.out.printf(Locale.US,
                "TrackB complete arm=%s summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
                ARM.name().toLowerCase(Locale.ROOT), output.summaryRows, NUM_WINDOWS,
    ...
    static void finishWindow(OutputOwner output, int window,
                             double ahlInput, double acidInput, double attAInput,
                             double attBInput, double repInput,
    ...
    static void writeSample(OutputOwner output, int window, double offset,
                            double ahlInput, double acidInput, double attAInput,
                            double attBInput, double repInput,
                            int births, int deaths, int clampDeaths, int acidDeaths, int oobDeaths,
        ...
        try {
            ...
            if (output.samples != null) {
                output.samples.write(String.format(Locale.US,
                        "%d;%d;%.2f;%.2f;%.2f;%.2f;%.2f;%.2f;%d;%d;%d;%d;%d;%d;%.9g;%.9g%n",
                        window, sample, offset, ahlInput, acidInput, attAInput, attBInput, repInput,
                        count, births, deaths, clampDeaths, acidDeaths, oobDeaths, meanL, lumSum));
                output.sampleRows++;
            }
            if (output.voxels != null) {
                ...
                if (ARM == Arm.BROWNIAN) {
                    ...
                    VoxelAnalyzer.VoxelReadout r = voxelAnalyzer.analyze(
                            ahl, acid, attractant, repellent, bacteria);
                    for (double v : r.ahlUm) output.voxels.write(format(v));
                    for (double v : r.pH) output.voxels.write(format(v));
                    for (double v : r.attConc) output.voxels.write(format(v));
                    for (double v : r.repConc) output.voxels.write(format(v));
    ...
    static void writeVoxelHeader(BufferedWriter w) throws IOException {
        StringBuilder h = new StringBuilder(
                "Window;Sample;TimeInWindow_s;Input_AHL;Input_Acid;Input_AttA;Input_AttB;Input_Rep");
        ...
        if (ARM == Arm.BROWNIAN) {
            ...
            for (int i = 0; i < states; i++) h.append(";Att_conc_").append(i);
            for (int i = 0; i < states; i++) h.append(";Rep_conc_").append(i);
            for (int i = 0; i < states; i++) h.append(";Den_").append(i);
    ...
    static void writeFeatureContract(File directory) throws IOException {
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write("stage6_status=PASS\n");
            w.write("stage7_status=FAIL\n");
            w.write("stage9_status=FAIL\n");
            w.write("bencha_lorenz_status=FAIL\n");
            w.write("bencha_mg_status=PASS\n");
            if (ARM == Arm.BROWNIAN) {
                ...
                w.write("ridge_features=Receiver_R / Mean_q (20x10); Lum_Mean / Mean_L (20x10); Input_Driven_Death (4x2); Den (20x10)\n");
                w.write("excluded=window AHL, occupancy, pH, Births, Total_Deaths, Clamp_Deaths, OOB, Population, Lum_Sum, voxel AHL_uM, voxel pH, voxel Att_conc, voxel Rep_conc, voxel Fraction_q, voxel Lum_Sum\n");
            }
        }
    }
    ...
    static void writeRunStatus(OutputOwner output) {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write(String.format(Locale.US,
                    "arm=%s%nseed=%d%nsummary_rows=%d%nsample_rows=%d%nvoxel_rows=%d%n",
    ...
    static void loadConfig(String path) {
        ...
        INPUT_ATTA_FILE = p.getProperty("input.atta.file", INPUT_ATTA_FILE);
        INPUT_ATTB_FILE = p.getProperty("input.attb.file", INPUT_ATTB_FILE);
        INPUT_REP_FILE = p.getProperty("input.rep.file", INPUT_REP_FILE);
        ...
        else if ("singlesite".equals(armName)) ARM = Arm.SINGLESITE;
        else throw new IllegalArgumentException(
                "arm must be brownian, silent, driven, or singlesite");
        if (ARM == Arm.SILENT) {
            AHL_SOURCE_RATE = 0.0;
