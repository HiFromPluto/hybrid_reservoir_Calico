...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanSweepMGE03.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanSweepMGE03.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanSweepMGE03.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanSweepMGE03.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanSweepMGE03.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanSweepMGE03.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanSweepMGE03.pHFromAcidMm(
                            BSimReservoirPlanSweepMGE03.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanSweepMGE03.ReservoirBacterium bacterium : bacteria) {
