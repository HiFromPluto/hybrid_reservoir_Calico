package BSimReservoirPlanSweepMGE03;
...
/**
 * SweepMGE03: Mackey-Glass one-step on the frozen three-geometry shortlist.
 ...
 * rate 1.28e8, K=1.6, tau_L=1500. AHL x in {200,500}; FLOW in {0, 0.25}. Do not rerun 8 um/s.
 */
public final class BSimReservoirPlanSweepMGE03 {
    ...
    static String OUTPUT_DIR = "results/mge03_center_f0p0_driven_seed101";
    static String INPUT_AHL_FILE = "input_ahl_mg200.txt";
    static String INPUT_ACID_FILE = "input_acid_held05_200.txt";
    static String RUN_LABEL = "mge03_center_f0p0_driven";
    ...
    static boolean SMOKE_NOT_EVIDENCE = false;
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close SweepMGE03 output", e);
            }
        }
    }

    public static void main(String[] args) {
        String configPath = args.length > 0 ? args[0] : "sim_config_mg.properties";
        ...
        try {
            ...
            throw new RuntimeException("Cannot open SweepMGE03 output", e);
        }
        ...
        System.out.printf(Locale.US,
                "SweepMGE03 complete arm=%s FLOW_SPEED=%.6f transit_s=%s Courant=%.6f chemical_boundary=%s bound=(%.1f,%.1f,%.1f) ahl=(%.1f,%.1f,%.1f) rate=%.0f summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
    ...
    static void assertClaimAhlRate() {
        if (Math.abs(CONFIGURED_AHL_SOURCE_RATE - 128000000.0) > 1e-3) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 AHL rate must be 128000000, printed %.6f", CONFIGURED_AHL_SOURCE_RATE));
        }
    }
    ...
    static void assertClaimAcidRate() {
        if (Math.abs(CONFIGURED_ACID_SOURCE_RATE - 2.0e11) > 1.0) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 acid rate must be 2e11, printed %.6g", CONFIGURED_ACID_SOURCE_RATE));
        }
    }
    ...
    static void assertAlphaEqualsDelta() {
        if (Math.abs(ALPHA_LUX - DELTA_LUX) > 1e-15) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 alpha=%.12g != delta=%.12g", ALPHA_LUX, DELTA_LUX));
        }
        if (!(DELTA_LUX > 0)) {
            throw new IllegalStateException("STOP SweepMGE03 delta must be positive");
        }
        if (Math.abs(ALPHA_LUX - 1.0 / 1500.0) > 1e-12
                || Math.abs(DELTA_LUX - 1.0 / 1500.0) > 1e-12) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 alpha=delta must be 1/1500, printed alpha=%.12g delta=%.12g",
                    ALPHA_LUX, DELTA_LUX));
        }
    }
    ...
    static void assertReadoutFrozen() {
        if (STATE_X != 20 || STATE_Y != 10 || STATE_Z != 1
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 readout must stay 20x10x1 + 4x2x1, printed %dx%dx%d / %dx%dx%d",
    ...
    static void assertBoundZ() {
        if (Math.abs(BOUND_Z - 10.0) > 1e-12) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 bound.z must be 10, printed %.6f", BOUND_Z));
        }
    }
    ...
    static void assertDtFrozen() {
        if (Math.abs(DT - 0.05) > 1e-12) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 dt must be 0.05, printed %.6f", DT));
        }
    }
    ...
    static void assertCfl() {
        ...
        if (DT > DT_CFL_MAX) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 dt=%.6f exceeds CFL dt_max=%.6f (dx=%.6f dy=%.6f). Do not lower dt.",
    ...
    static void requireInside(String name, double x, double y, double z) {
        if (x < 0 || x > BOUND_X || y < 0 || y > BOUND_Y || z < 0 || z > BOUND_Z) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 %s=(%.4f, %.4f, %.4f) outside [0,bound]=(%.1f, %.1f, %.1f)",
    ...
    static void assertClaimBox() {
        if (Math.abs(BOUND_X - 1000.0) > 1e-9
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 bounds must be 1000x500x10, printed %.4f x %.4f x %.4f",
                    BOUND_X, BOUND_Y, BOUND_Z));
        }
        if (GRID_X != 50 || GRID_Y != 25 || GRID_Z != 1) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 grid must be 50x25x1, printed %dx%dx%d",
        ...
        if (!allowedAhlX
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 AHL must be (200|500, 250, 5), printed (%.4f, %.4f, %.4f)",
                    AHL_SOURCE_POSITION.x, AHL_SOURCE_POSITION.y, AHL_SOURCE_POSITION.z));
        }
        if (Math.abs(ACID_SOURCE_POSITION.x - 300.0) > 1e-9
            ...
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 acid must be (300, 375, 5), printed (%.4f, %.4f, %.4f)",
    ...
    static void assertFlowCourant() {
        if (FLOW_SPEED < 0) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 FLOW_SPEED must be >= 0, printed %.6f", FLOW_SPEED));
        }
        ...
        if (CHEMICAL_COURANT >= 1.0) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 chemical Courant=%.6f >= 1 (FLOW_SPEED=%.6f dt=%.6f dx=%.6f). Do not lower dt.",
                    CHEMICAL_COURANT, FLOW_SPEED, DT, FIELD_DX));
        }
        double[] allowed = {0.0, 0.25};
        ...
        if (Math.abs(FLOW_SPEED - 8.0) < 1e-6) {
            throw new IllegalStateException(
                    "STOP SweepMGE03 do not rerun 8 um/s; cite SweepS5 occupancy-dead NARMA anchor");
        }
        if (!flowOk) {
            throw new IllegalStateException(String.format(Locale.US,
                    "STOP SweepMGE03 FLOW_SPEED=%.6f is not in the frozen MG geometry table {0, 0.25}", FLOW_SPEED));
        }
        CHEMICAL_BOUNDARY = FLOW_SPEED > 0 ? "OUTFLOW" : "NO_FLUX";
        if (FLOW_SPEED > 0 && !"OUTFLOW".equals(CHEMICAL_BOUNDARY)) {
            throw new IllegalStateException("STOP SweepMGE03 positive flow must be OUTFLOW");
        }
        if (FLOW_SPEED == 0 && !"NO_FLUX".equals(CHEMICAL_BOUNDARY)) {
            throw new IllegalStateException("STOP SweepMGE03 zero flow must be NO_FLUX");
        }
    }
    ...
    static void printGeometryBanner() {
        System.out.printf(Locale.US,
                "SweepMGE03 bound.x=%.6f bound.y=%.6f bound.z=%.6f%n", BOUND_X, BOUND_Y, BOUND_Z);
        System.out.printf(Locale.US,
                "SweepMGE03 grid.x=%d grid.y=%d grid.z=%d dx=%.6f dy=%.6f CFL_dt_max=%.6f dt=%.6f%n",
                GRID_X, GRID_Y, GRID_Z, FIELD_DX, FIELD_DY, DT_CFL_MAX, DT);
        System.out.printf(Locale.US,
                "SweepMGE03 field.ahl.source.x=%.6f field.ahl.source.y=%.6f field.ahl.source.z=%.6f rate=%.0f%n",
                AHL_SOURCE_POSITION.x, AHL_SOURCE_POSITION.y, AHL_SOURCE_POSITION.z, AHL_SOURCE_RATE);
        System.out.printf(Locale.US,
                "SweepMGE03 field.acid.source.x=%.6f field.acid.source.y=%.6f field.acid.source.z=%.6f rate=%.6g%n",
                ACID_SOURCE_POSITION.x, ACID_SOURCE_POSITION.y, ACID_SOURCE_POSITION.z, ACID_SOURCE_RATE);
        System.out.printf(Locale.US,
                "SweepMGE03 field.att0.x=%.6f field.att0.y=%.6f field.att0.z=%.6f%n",
                ATT0_POSITION.x, ATT0_POSITION.y, ATT0_POSITION.z);
        System.out.printf(Locale.US,
                "SweepMGE03 field.att2.x=%.6f field.att2.y=%.6f field.att2.z=%.6f%n",
                ATT2_POSITION.x, ATT2_POSITION.y, ATT2_POSITION.z);
        System.out.printf(Locale.US,
                "SweepMGE03 initial.pop=%d carrying.capacity=%d luminescence.alpha=%.12g luminescence.delta=%.12g%n",
                INITIAL_POP, CARRYING_CAPACITY, ALPHA_LUX, DELTA_LUX);
        System.out.printf(Locale.US,
                "SweepMGE03 FLOW_SPEED=%.6f transit_s=%s chemical_Courant=%.6f CFL_dt_max_flow=%s chemical_boundary=%s%n",
                FLOW_SPEED, formatTransit(TRANSIT_S), CHEMICAL_COURANT, formatTransit(DT_CFL_MAX_FLOW),
                CHEMICAL_BOUNDARY);
        if (FLOW_SPEED > 0) {
            System.out.println("SweepMGE03 advect=on Stokes=on");
        } else {
            System.out.println("SweepMGE03 advect=off Stokes=off");
        }
    }
    ...
    static void writeSourcePositions(File directory) throws IOException {
        ...
        try (BufferedWriter w = new BufferedWriter(
            ...
            w.write("note=SweepMGE03: MG three-geometry scout; claim box 1000x500x10; AHL rate 1.28e8; K=1.6; tau_L=1500; SweepS5 Stage 6 upwind when FLOW>0; no recycling; AHL x in {200,500}; FLOW in {0,0.25}; no 8 um/s rerun\n");
        }
        if (SMOKE_NOT_EVIDENCE) {
            try (BufferedWriter w = new BufferedWriter(
                    new FileWriter(new File(directory, "SMOKE_NOT_EVIDENCE.txt")))) {
                w.write("This directory is HybridDish smoke timing only. Do not score as Mackey-Glass.\n");
            }
        }
    }
    ...
    static void loadConfig(String path) {
        ...
        SMOKE_NOT_EVIDENCE = Boolean.parseBoolean(
                p.getProperty("sweep.smoke.not.mg", String.valueOf(SMOKE_NOT_EVIDENCE)));
