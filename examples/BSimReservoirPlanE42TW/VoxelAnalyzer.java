...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanE42TW.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanE42TW.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanE42TW.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanE42TW.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanE42TW.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanE42TW.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanE42TW.pHFromAcidMm(
                            BSimReservoirPlanE42TW.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanE42TW.ReservoirBacterium bacterium : bacteria) {
