...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanWaveform.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanWaveform.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanWaveform.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanWaveform.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanWaveform.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanWaveform.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanWaveform.pHFromAcidMm(
                            BSimReservoirPlanWaveform.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanWaveform.ReservoirBacterium bacterium : bacteria) {
