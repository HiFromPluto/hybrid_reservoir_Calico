...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimHybridDish.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimHybridDish.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimHybridDish.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimHybridDish.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimHybridDish.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimHybridDish.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimHybridDish.pHFromAcidMm(
                            BSimHybridDish.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimHybridDish.ReservoirBacterium bacterium : bacteria) {
