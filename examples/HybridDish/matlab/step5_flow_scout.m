function T = step5_flow_scout(varargin)
%STEP5_FLOW_SCOUT  Closed ridge + occupancy for Sweep Step 5.
%   Claim seed 111 is Narma10b on disk (no BSim rerun).
%   Flow 8 um/s reads SweepS5 results. ridge_closed unchanged.
%   (F) is expected under flow and is not a promote.

    here = fileparts(mfilename('fullpath'));
    addpath(here);
    examples = fileparts(fileparts(here));
    narmaDir = fullfile(examples, 'BSimReservoirPlanNarma10b');
    sweepDir = fullfile(examples, 'BSimReservoirPlanSweepS5');
    ...
    y = target.y_next(:);
    u = target.u(:);
    B = baselines(y, 'Label', 'NARMA-10 SweepS5');
    ...
    specs = {
        ...
        'flow',  8, 8*0.05/20, ...
            fullfile(p.Results.SweepDir, 'results', 's5_flow_driven_seed111'), ...
            fullfile(p.Results.SweepDir, 'results', 's5_flow_brownian_seed111')
        };
    ...
    for i = 1:size(specs, 1)
        ...
        if ~isfile(fullfile(drivenDir, 'voxels.csv'))
            error('step5:missing', 'STOP missing %s', fullfile(drivenDir, 'voxels.csv'));
        end
        if ~isfile(fullfile(brownDir, 'voxels.csv'))
            error('step5:missing', 'STOP missing %s', fullfile(brownDir, 'voxels.csv'));
        end
        fprintf('\n=== flow %s %.0f um/s Courant=%.4f seed 111 ===\n', tag, speed, courant);
        Td = load_voxels(drivenDir);
        X408 = window_features(Td, 'biology');
        if size(X408, 2) ~= 408
            error('step5:nfeat', 'STOP %s biology n_features=%d (readout voxel count changed)', ...
                tag, size(X408, 2));
        end
        XF = window_features(Td, 'field');
        occ = occupancy_from_table(Td, u);
        meanPop = mean_population(drivenDir);
        fprintf('  occupancy %s  r(mean_R,u)=%.4f  mean_R=%.4f  mean_AHL=%.4f  frac>0.5=%.4f  r(AHL,u)=%.4f  mean_pop=%.1f\n', ...
            occ.label, occ.r_meanR_u, occ.mean_R, occ.mean_AHL, occ.frac_R_gt_0_5, occ.r_meanAHL_u, meanPop);
        ...
        meanPopB = mean_population(brownDir);
        fprintf('  brownian Den n=%d lambda=%.4g NRMSE=%.4f  mean_pop=%.1f\n', ...
            size(XD,2), fitB.lambda, fitB.test_nrmse, meanPopB);

        rows(end+1, :) = {tag, speed, 111, 'driven', size(X408,2), fitD.lambda, ...
            fitD.test_nrmse, fitD.test_r2, occ.r_meanR_u, occ.mean_R, ...
            occ.mean_AHL, occ.frac_R_gt_0_5, occ.label, meanPop, courant}; %#ok<AGROW>
        rows(end+1, :) = {tag, speed, 111, 'field', size(XF,2), fitF.lambda, ...
            fitF.test_nrmse, fitF.test_r2, occ.r_meanR_u, occ.mean_R, ...
            occ.mean_AHL, occ.frac_R_gt_0_5, occ.label, meanPop, courant}; %#ok<AGROW>
        rows(end+1, :) = {tag, speed, 111, 'brownian', size(XD,2), fitB.lambda, ...
            fitB.test_nrmse, fitB.test_r2, NaN, NaN, NaN, NaN, 'n/a', meanPopB, courant}; %#ok<AGROW>
    end

    T = cell2table(rows, 'VariableNames', {
        'flow_tag', 'flow_um_s', 'seed', 'arm', 'n_features', 'lambda', ...
        'test_nrmse', 'test_r2', 'r_meanR_u', 'mean_R', 'mean_AHL', ...
        'frac_R_gt_0_5', 'occupancy', 'mean_pop', 'courant'});
    if ~exist(p.Results.OutDir, 'dir')
        mkdir(p.Results.OutDir);
    end
    outCsv = fullfile(p.Results.OutDir, 'step5_flow_scout.csv');
    writetable(T, outCsv);
    fprintf('\nWrote %s (%d rows)\n', outCsv, height(T));
...
function occ = occupancy_from_table(T, u)
    ...
    if numel(u) ~= nW
        error('step5:u', 'u length %d vs windows %d', numel(u), nW);
    end
...
function m = mean_population(runDir)
    f = fullfile(runDir, 'window_summary.csv');
    if ~isfile(f)
        m = NaN;
        return
    end
    W = readtable(f, 'Delimiter', ';', 'FileType', 'text');
    if ~ismember('Population', W.Properties.VariableNames)
        m = NaN;
        return
    end
    m = mean(W.Population, 'omitnan');
end
...
function check_claim_sanity(T)
    ...
    hit = strcmp(T.flow_tag, 'claim') & strcmp(T.arm, 'driven');
    rR = T.r_meanR_u(hit);
    mR = T.mean_R(hit);
...
function print_field_gap(T)
    Cd = pick(T, 'claim', 'driven');
    Cf = pick(T, 'claim', 'field');
    Ld = pick(T, 'flow', 'driven');
    Lf = pick(T, 'flow', 'field');
    Lb = pick(T, 'flow', 'brownian');
    gapC = Cf - Cd;
    gapL = Lf - Ld;
    fprintf('FIELD_GAP claim: %.4f   FIELD_GAP flow: %.4f\n', gapC, gapL);
    if Lf < Cf
        fprintf('EXPECTED: field improved\n');
    else
        fprintf('UNEXPECTED: field got worse; do not then change speed\n');
    end
    if Ld >= Lf
        fprintf('FIELD vs driven: FIELD_BEATS_DRIVEN\n');
    else
        fprintf('FIELD vs driven: DRIVEN_STILL_BEATS_FIELD\n');
    end
    fprintf('flow driven=%.4f field=%.4f brownian=%.4f  (B) Ld>=Lb -> %d\n', ...
        Ld, Lf, Lb, double(Ld >= Lb));
end

function v = pick(T, tag, arm)
    hit = strcmp(T.flow_tag, tag) & strcmp(T.arm, arm) & T.seed == 111;
    if sum(hit) ~= 1
        error('step5:pick', 'expected 1 row %s %s', tag, arm);
    end
    v = T.test_nrmse(hit);
end
