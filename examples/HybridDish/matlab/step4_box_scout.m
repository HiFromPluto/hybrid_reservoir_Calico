function T = step4_box_scout(varargin)
%STEP4_BOX_SCOUT  Closed ridge + occupancy for Sweep Step 4.
%   Claim seed 111 is Narma10b on disk (no BSim rerun).
%   Small/large boxes read SweepS4 results. ridge_closed unchanged.
%   K, rate, tau_L, and readout voxel count are not knobs.

    here = fileparts(mfilename('fullpath'));
    addpath(here);
    examples = fileparts(fileparts(here));
    narmaDir = fullfile(examples, 'BSimReservoirPlanNarma10b');
    sweepDir = fullfile(examples, 'BSimReservoirPlanSweepS4');
    ...
    y = target.y_next(:);
    u = target.u(:);
    B = baselines(y, 'Label', 'NARMA-10 SweepS4');
    fprintf('  persistence=%.4f  intercept=%.4f\n', ...
        B.persistence_nrmse, B.intercept_nrmse);

    specs = {
        'small', 500, 250, 25, 13, ...
            fullfile(p.Results.SweepDir, 'results', 's4_small_driven_seed111'), ...
            fullfile(p.Results.SweepDir, 'results', 's4_small_brownian_seed111')
        'claim', 1000, 500, 50, 25, ...
            fullfile(p.Results.NarmaDir, 'results', 'narma10b_driven_seed111'), ...
            fullfile(p.Results.NarmaDir, 'results', 'narma10b_brownian_seed111')
        'large', 2000, 1000, 100, 50, ...
            fullfile(p.Results.SweepDir, 'results', 's4_large_driven_seed111'), ...
            fullfile(p.Results.SweepDir, 'results', 's4_large_brownian_seed111')
        };
    ...
    for i = 1:size(specs, 1)
        ...
        if ~isfile(fullfile(drivenDir, 'voxels.csv'))
            error('step4:missing', 'STOP missing %s', fullfile(drivenDir, 'voxels.csv'));
        end
        if ~isfile(fullfile(brownDir, 'voxels.csv'))
            error('step4:missing', 'STOP missing %s', fullfile(brownDir, 'voxels.csv'));
        end
        fprintf('\n=== box %s %dx%d grid %dx%d seed 111 ===\n', tag, bx, by, gx, gy);
        Td = load_voxels(drivenDir);
        X408 = window_features(Td, 'biology');
        if size(X408, 2) ~= 408
            error('step4:nfeat', 'STOP %s biology n_features=%d (readout voxel count changed)', ...
                tag, size(X408, 2));
        end
        XF = window_features(Td, 'field');
        occ = occupancy_from_table(Td, u);
        meanPop = mean_population(drivenDir);
        fprintf('  occupancy %s  r(mean_R,u)=%.4f  mean_R=%.4f  mean_AHL=%.4f  frac>0.5=%.4f  mean_pop=%.1f\n', ...
            occ.label, occ.r_meanR_u, occ.mean_R, occ.mean_AHL, occ.frac_R_gt_0_5, meanPop);
        ...
        rows(end+1, :) = {tag, bx, by, gx, gy, 111, 'driven', size(X408,2), fitD.lambda, ...
            fitD.test_nrmse, fitD.test_r2, occ.r_meanR_u, occ.mean_R, ...
            occ.mean_AHL, occ.frac_R_gt_0_5, occ.label, meanPop}; %#ok<AGROW>
        rows(end+1, :) = {tag, bx, by, gx, gy, 111, 'field', size(XF,2), fitF.lambda, ...
            fitF.test_nrmse, fitF.test_r2, occ.r_meanR_u, occ.mean_R, ...
            occ.mean_AHL, occ.frac_R_gt_0_5, occ.label, meanPop}; %#ok<AGROW>
        rows(end+1, :) = {tag, bx, by, gx, gy, 111, 'brownian', size(XD,2), fitB.lambda, ...
            fitB.test_nrmse, fitB.test_r2, NaN, NaN, NaN, NaN, 'n/a', meanPopB}; %#ok<AGROW>
    end

    T = cell2table(rows, 'VariableNames', {
        'box_tag', 'bound_x', 'bound_y', 'grid_x', 'grid_y', 'seed', 'arm', ...
        'n_features', 'lambda', 'test_nrmse', 'test_r2', ...
        'r_meanR_u', 'mean_R', 'mean_AHL', 'frac_R_gt_0_5', 'occupancy', 'mean_pop'});
    if ~exist(p.Results.OutDir, 'dir')
        mkdir(p.Results.OutDir);
    end
    outCsv = fullfile(p.Results.OutDir, 'step4_box_scout.csv');
    writetable(T, outCsv);
    fprintf('\nWrote %s (%d rows)\n', outCsv, height(T));
...
function occ = occupancy_from_table(T, u)
    ...
    if numel(u) ~= nW
        error('step4:u', 'u length %d vs windows %d', numel(u), nW);
    end
...
function check_claim_sanity(T)
    ...
    hit = strcmp(T.box_tag, 'claim') & strcmp(T.arm, 'driven');
    rR = T.r_meanR_u(hit);
    mR = T.mean_R(hit);
...
function v = pick(T, tag, arm)
    hit = strcmp(T.box_tag, tag) & strcmp(T.arm, arm) & T.seed == 111;
    if sum(hit) ~= 1
        error('step4:pick', 'expected 1 row %s %s', tag, arm);
    end
    v = T.test_nrmse(hit);
end
