#!/usr/bin/env python3
"""Generate the frozen Waveform2 equal-histogram temporal-order sequences.

Track E1. Strict temporal-order classification. Classes are ORDER_A,
ORDER_B, ORDER_C — not sine/square/triangle. Templates are permutations
of one amplitude multiset. Do not retune templates after seeing reservoir
AUC. Class shuffle uses random.Random(20260817). Phase assignment uses
a documented subseed that is incremented only if split constraints fail.
"""
...
TEMPLATES = (
    (MID, HI, MAXV, HI, MID, LO, MINV, LO),  # ORDER_A sine-order permutation
    (MAXV, HI, HI, MID, MID, LO, LO, MINV),  # ORDER_B step-order permutation
    (MINV, LO, MID, HI, MAXV, HI, MID, LO),  # ORDER_C ramp-order permutation
)
...
def main():
    ...
    same_orbit = {
        f"{CLASS_NAMES[i]}=={CLASS_NAMES[j]}": bool(orbits[i] & orbits[j])
        for i in range(N_CLASSES)
        for j in range(i + 1, N_CLASSES)
    }
    print("CYCLIC_ORBIT_OVERLAP")
    for key, value in same_orbit.items():
        print(f"  {key}: {value}")
    ...
    ahl_path = directory / "input_ahl_waveform2_400.txt"
    ahl_path.write_text(
        "# Frozen Waveform2 AHL. Equal-histogram temporal-order templates.\n"
        "# Classes ORDER_A/ORDER_B/ORDER_C. Shared 8-value multiset. No extra noise.\n"
    ...
    acid_path.write_text(
        "# Frozen Waveform2 acid hold. Every analysis window is 0.5. Off in warmup (Java).\n"
    ...
    print(
        "washout ORDER_A={0} ORDER_B={1} ORDER_C={2}".format(
            washout.count(0), washout.count(1), washout.count(2)
        )
    )
    print(
        "train ORDER_A={0} ORDER_B={1} ORDER_C={2}".format(
            train.count(0), train.count(1), train.count(2)
        )
    )
    print(
        "test ORDER_A={0} ORDER_B={1} ORDER_C={2}".format(
