...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanWaveform2.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanWaveform2.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanWaveform2.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanWaveform2.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanWaveform2.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanWaveform2.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanWaveform2.pHFromAcidMm(
                            BSimReservoirPlanWaveform2.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanWaveform2.ReservoirBacterium bacterium : bacteria) {
