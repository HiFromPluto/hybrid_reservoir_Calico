#!/usr/bin/env python3
"""Evaluate Waveform2 temporal-order classification on the frozen claim dish.

Does not rewrite Waveform GATE_EVIDENCE.md. Kinetics are not retuned.
Primary numbers are block-level. u-only audit must PASS before BSim.
"""
...
N_CLASSES = 3
CLASS_NAMES = ("ORDER_A", "ORDER_B", "ORDER_C")
CHANCE_ACC = 1.0 / 3.0
CHANCE_AUC = 0.5
...
TEMPLATES = (
    ...
    (MINV, LO, MID, HI, MAXV, HI, MID, LO),
)

CLASS_VECTOR = (
    "1,1,2,0,1,2,0,0,2,1,0,2,0,0,0,1,1,0,1,0,1,2,1,0,1,1,1,2,0,2,2,2,0,2,"
    "1,1,0,1,0,1,2,2,2,2,1,0,0,2,0,0"
)
PHASE_VECTOR = (
    "0,3,2,2,7,4,7,6,1,3,5,3,7,0,2,1,6,3,0,4,7,2,5,1,4,1,2,4,6,6,7,5,1,0,"
    "0,4,4,5,1,6,5,0,3,7,7,7,2,6,6,0"
)
CLASS_SHA = "83aab1be5b5499566c802d3be58f918c572fb77b2a50baf6b9c6de911ca9499d"
PHASE_SHA = "5cf285dc94883d4b892fa00b22dd751a4f4d5c3ea672787f2e0d02b57f98c73a"
U_SHA = "8f47d9f30d888b0e223e23b8804b8d875187b800f283a166d15ab47a44663d31"
...
def verify_frozen_inputs():
    u_file = load_sequence(HERE / "input_ahl_waveform2_400.txt")
    digest = sha256_u(u_file)
    if digest != U_SHA:
        raise ValueError(f"AHL SHA-256 {digest} does not match frozen {U_SHA}")
    y, block, phase, u_csv, label_rows = load_labels(HERE / "waveform2_labels.csv")
...
def u_only_baselines(frozen):
    ...
    for name, X in features.items():
        ...
        rows.append({
            ...
            "ORDER_A_auc": metrics["block_per_class_ovr_auc"]["ORDER_A"],
            "ORDER_B_auc": metrics["block_per_class_ovr_auc"]["ORDER_B"],
            "ORDER_C_auc": metrics["block_per_class_ovr_auc"]["ORDER_C"],
...
def write_u_only(audit):
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=[
            ...
            "block_accuracy", "ORDER_A_auc", "ORDER_B_auc", "ORDER_C_auc",
    ...
    lines = [
        "# Waveform2 u-only baselines",
        "",
        "Block-level one-vs-rest ridge on the frozen equal-histogram",
        "templates. Hashes were recorded in PROTOCOL.md before this ridge.",
        "This is not reservoir skill.",
        "",
        f"## Result: {status}",
        "",
        ...
        "",
        "| Baseline | Features | Lambda | Block macro AUC | Accuracy | ORDER_A | ORDER_B | ORDER_C | Pass |",
        "|---|---|---|---|---|---|---|---|---|",
    ]
    for row in audit["rows"]:
        lines.append(
            ...
            f"{row['ORDER_A_auc']:.4f} | {row['ORDER_B_auc']:.4f} | "
            f"{row['ORDER_C_auc']:.4f} | {row['pass']} |"
        )
    lines.extend([
        "",
        ...
        "",
        "ORDER_A and ORDER_C occupy the same cyclic orbit. If RAW_U_8 is",
...
def java_constraints():
    java_path = HERE / "BSimReservoirPlanWaveform2.java"
...
def available_run(arm, seed):
    path = HERE / "results" / f"waveform2_{arm}_seed{seed}"
...
def main():
    ...
    parser.add_argument("--smoke", default=str(HERE / "results" / "waveform2_smoke"))
    ...
    if not audit["overall_pass"]:
        print("STOP: u-only audit FAIL. Do not interpret BSim as Waveform2 skill.")
        raise SystemExit(1)
    ...
    for arm in ("brownian", "silent", "driven"):
        ...
        for seed in seeds:
            ...
            rows.append({
                ...
                "ORDER_A_auc": cls["block_per_class_ovr_auc"]["ORDER_A"],
                "ORDER_B_auc": cls["block_per_class_ovr_auc"]["ORDER_B"],
                "ORDER_C_auc": cls["block_per_class_ovr_auc"]["ORDER_C"],
    ...
    write_scores_csv(HERE / "results" / "waveform2_scores.csv", rows)
    (HERE / "results" / "waveform2_gate_evidence.json").write_text(
        json.dumps({
            "u_only": audit,
    ...
    print(f"scores={HERE / 'results' / 'waveform2_scores.csv'}")
