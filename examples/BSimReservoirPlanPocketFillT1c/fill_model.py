#!/usr/bin/env python3
"""PocketFill-T1c reduced field + mean-field Danino ODE.

Door vs circuit. Same QS_* as T1/T1b. No NARMA. No QS_KMLA hunt.
"""
...
def _d_map(shape, pocket: np.ndarray) -> np.ndarray:
    d = np.full(shape, D0, dtype=float)
    d[pocket] = D0
    return d
...
def _finish(active, pocket, bus, k_decay: float, kind: str, bus_flow: float) -> Geometry:
    idx, n = _index_active(active)
    M = lil_matrix((n, n), dtype=float)
    _assemble_diffusion(M, idx, active, _d_map(active.shape, pocket))
    _add_decay(M, n, k_decay)
    if bus_flow:
        _add_upwind_x(M, idx, bus, bus_flow)
...
def make_closed_geometry(k_decay: float) -> Geometry:
    nx = int(round(POCKET_LX / DX))
    ny = int(round(POCKET_LY / DY))
    active, pocket, bus = _empty_grid(nx, ny)
    pocket[:, :] = True
    active[:, :] = True
    return _finish(active, pocket, bus, k_decay, "closed", 0.0)


def make_flush_geometry(k_decay: float) -> Geometry:
    ...
    y0b = float(DISH["flush_bus_y0_um"])
    bus_len = float(DISH["bus_length_um"])
    bus_w = float(DISH["bus_width_um"])
    for i in range(nx):
        x = (i + 0.5) * DX
        for j in range(ny):
            y = (j + 0.5) * DY
            in_p = (x0p <= x < x0p + POCKET_LX) and (y0p <= y < y0p + POCKET_LY)
            in_b = (x0b <= x < x0b + bus_len) and (y0b <= y < y0b + bus_w)
            pocket[i, j] = in_p
            bus[i, j] = in_b
            active[i, j] = in_p or in_b
    return _finish(active, pocket, bus, k_decay, "flush", 3.0)


def make_w20_geometry(k_decay: float) -> Geometry:
    nx = int(round(float(DISH["bus_length_um"]) / DX))
    ny = int(round((POCKET_LY + float(DISH["neck_length_um"]) + float(DISH["bus_width_um"])) / DY))
    active, pocket, bus = _empty_grid(nx, ny)
    neck = np.zeros((nx, ny), dtype=bool)
    x0p = float(DISH["pocket_x0_um"])
    y0p = float(DISH["pocket_y0_um"])
    x0b = float(DISH["bus_x0_um"])
    y0b = float(DISH["necked_bus_y0_um"])
    neck_y0 = float(DISH["neck_y0_um"])
    ln = float(DISH["neck_length_um"])
    w = float(DISH["neck_w_um"])
    x_mid = x0p + 0.5 * POCKET_LX
    nx0, nx1 = x_mid - 0.5 * w, x_mid + 0.5 * w
    bus_len = float(DISH["bus_length_um"])
    bus_w = float(DISH["bus_width_um"])
    for i in range(nx):
        x = (i + 0.5) * DX
        for j in range(ny):
            ...
            in_p = (x0p <= x < x0p + POCKET_LX) and (y0p <= y < y0p + POCKET_LY)
            in_b = (x0b <= x < x0b + bus_len) and (y0b <= y < y0b + bus_w)
            in_n = (nx0 <= x < nx1) and (neck_y0 <= y < neck_y0 + ln)
            if in_p and in_n:
                raise RuntimeError(f"neck stole pocket voxel ({i},{j})")
            pocket[i, j] = in_p
            neck[i, j] = in_n
            bus[i, j] = in_b
            active[i, j] = in_p or in_n or in_b
    geom = _finish(active, pocket, bus, k_decay, "w20", 3.0)
    return geom


def make_geometry(kind: str) -> Geometry:
    if kind == "closed":
        return make_closed_geometry(K_DANINO)
    if kind == "flush":
        return make_flush_geometry(K_DANINO)
    if kind == "w20":
        return make_w20_geometry(K_DANINO)
...
def integrate_volumetric(geom: Geometry, duration: float, sample_dt: float = 60.0) -> dict:
    ...
    return {"t": sol.t, "C": sol.y[:n].T, "Y": sol.y[n:].T, "sol": sol}


def field_budget(geom: Geometry, sol, duration: float, dt: float = 0.25) -> dict:
    n = geom.n
    n_p = len(geom.pocket_idx)
