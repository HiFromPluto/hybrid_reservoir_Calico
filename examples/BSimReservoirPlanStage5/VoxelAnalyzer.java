...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanStage5.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanStage5.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanStage5.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanStage5.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanStage5.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanStage5.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanStage5.pHFromAcidMm(
                            BSimReservoirPlanStage5.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanStage5.ReservoirBacterium bacterium : bacteria) {
