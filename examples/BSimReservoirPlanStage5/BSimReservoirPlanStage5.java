package BSimReservoirPlanStage5;
...
/**
 * Spatial Plan Stage 5. Copies frozen Stage 4, then adds a per-cell luminescence
 * state downstream of the frozen Stage 3B receiver. Stage 4 remains FAIL.
 * Receiver, acid source, K_MAX, and clamp are not retuned.
 */
public final class BSimReservoirPlanStage5 {
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close Stage 5 output", e);
            }
        }
    }

    public static void main(String[] args) {
        String configPath = args.length > 0 ? args[0] : "sim_config_stage5.properties";
        ...
        try {
            ...
            throw new RuntimeException("Cannot open Stage 5 output", e);
        }
        ...
        System.out.printf(Locale.US,
                "Stage5 complete summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d alpha_lux=%.9g t95_L=%.3f%n",
    ...
    static void finishWindow(OutputOwner output, int window, double ahlInput, double acidInput,
        ...
        try {
            output.summary.write(String.format(Locale.US,
                    ...
                    n == 0 ? Double.NaN : lum[0],
                    quantile(lum, .1), quantile(lum, .5),
                    quantile(lum, .9), n == 0 ? Double.NaN : lum[n - 1],
                    accumulator.lumSum,
    ...
    static void writeSample(OutputOwner output, int window, double offset,
        ...
        try {
            ...
            double meanL = bacteria.isEmpty() ? Double.NaN : lumSum / bacteria.size();
