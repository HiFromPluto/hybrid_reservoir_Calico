...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanNarma10b.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanNarma10b.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanNarma10b.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanNarma10b.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanNarma10b.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanNarma10b.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanNarma10b.pHFromAcidMm(
                            BSimReservoirPlanNarma10b.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanNarma10b.ReservoirBacterium bacterium : bacteria) {
