...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanWashoutReset.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanWashoutReset.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanWashoutReset.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanWashoutReset.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanWashoutReset.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanWashoutReset.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanWashoutReset.pHFromAcidMm(
                            BSimReservoirPlanWashoutReset.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanWashoutReset.ReservoirBacterium bacterium : bacteria) {
