#!/usr/bin/env python3
"""Evaluate Waveform2c aligned equal-histogram temporal-order classification.

Does not rewrite Waveform GATE_EVIDENCE.md, Waveform2, or Waveform2b.
Kinetics are not retuned. Phase is identically 0. u-only must PASS before BSim.
"""
...
CLASS_VECTOR = (
    "0,0,2,0,1,1,1,2,2,0,1,2,0,1,1,0,0,2,1,1,2,2,1,0,1,1,2,0,0,0,1,0,2,2,"
    "1,0,0,0,2,0,1,0,2,2,1,2,2,1,0,1"
)
PHASE_VECTOR = (
...
PHASE_SHA = "ac6e5db2e0d7e63815992a3ca671b395e6bc533228054cffa27c17f35fa89cb8"
...
def template_u(cls, phase, w):
    if int(phase) != 0:
        raise ValueError("Waveform2c phase must be 0")
...
def verify_frozen_inputs():
    u_file = load_sequence(HERE / "input_ahl_waveform2c_400.txt")
    digest = sha256_u(u_file)
    if digest != U_SHA:
        raise ValueError(f"AHL SHA-256 {digest} does not match frozen {U_SHA}")
    y, block, phase, u_csv, label_rows = load_labels(HERE / "waveform2c_labels.csv")
    ...
    if any(int(p) != 0 for p in phase):
        raise ValueError("Waveform2c phase must be identically 0")
    if any(round(TEMPLATES[c][0], 12) != MID for c in range(N_CLASSES)):
        raise ValueError("canonical templates must start at MID")
...
def block_moments_from_u(block_u):
    ...
    return {
        ...
        "CYCLIC_MATCH": cyclic_match_features(block_u),
    }


def cyclic_match_features(block_u):
    """Min L2 to each class orbit. Diagnostic, not a gate."""
    templates = np.asarray(TEMPLATES, dtype=float)
    feats = np.zeros((block_u.shape[0], N_CLASSES), dtype=float)
    for b, vec in enumerate(block_u):
        for c in range(N_CLASSES):
            t = templates[c]
            dists = [
                float(np.linalg.norm(vec - np.concatenate([t[p:], t[:p]])))
                for p in range(WINDOWS_PER_BLOCK)
            ]
            feats[b, c] = min(dists)
    return feats
...
def u_only_baselines(frozen):
    ...
    for name, X in features.items():
        ...
        if name == "CYCLIC_MATCH":
            criterion = "diagnostic_not_a_gate"
            passed = True
        elif name == "RAW_U_8":
            passed = auc >= 0.90
            raw_ok = passed
    ...
    return {
        ...
        "cyclic_auc": next(r["block_macro_ovr_auc"] for r in rows if r["baseline"] == "CYCLIC_MATCH"),
    }
...
def write_u_only(audit):
    ...
    lines = [
        "# Waveform2c u-only baselines",
        "",
        "Block-level one-vs-rest ridge on aligned equal-histogram",
        "templates (phase identically 0). Hashes were recorded in",
        "PROTOCOL.md before this ridge. CYCLIC_MATCH is diagnostic,",
        "not a gate. Waveform2/2b remain retained negatives.",
        "",
        f"## Result: {status}",
        "",
    ...
    lines.extend([
        ...
        "lie within 0.15 of 0.5. RAW_U_8 must be >= 0.90. CYCLIC_MATCH is",
        "diagnostic only. Phase is identically 0. If RAW_U_8 is below 0.90,",
        "the three aligned 8-vectors are not linearly distinct.",
        "",
    ])
...
def patch_protocol_u_only(audit):
    ...
    replacement = (
        ...
        f"- CYCLIC_MATCH {audit['cyclic_auc']:.4f} (diagnostic)\n"
        "\n"
    ...
    if not audit["overall_pass"]:
        if not audit["raw_ok"]:
            replacement += (
                "\nSTOP. RAW_U_8 is below 0.90. The three aligned 8-vectors\n"
                "are not linearly distinct. Do not run BSim. Do not rewrite\n"
                "templates after that score.\n"
            )
...
def java_constraints():
    java_path = HERE / "BSimReservoirPlanWaveform2c.java"
...
def available_run(arm, seed):
    path = HERE / "results" / f"waveform2c_{arm}_seed{seed}"
...
def main():
    ...
    parser.add_argument("--smoke", default=str(HERE / "results" / "waveform2c_smoke"))
    ...
    if not audit["overall_pass"]:
        print("STOP: u-only audit FAIL. Do not interpret BSim as Waveform2c skill.")
        raise SystemExit(1)
    ...
    write_scores_csv(HERE / "results" / "waveform2c_scores.csv", rows)
    (HERE / "results" / "waveform2c_gate_evidence.json").write_text(
        json.dumps({
            "u_only": audit,
    ...
    print(f"scores={HERE / 'results' / 'waveform2c_scores.csv'}")
