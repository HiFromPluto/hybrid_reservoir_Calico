...
public final class VoxelAnalyzer {
    ...
    public void recordDeath(Vector3d p, BSimReservoirPlanWaveform2c.DeathCause cause) {
        int bin = countBinFor(p);
        if (cause == BSimReservoirPlanWaveform2c.DeathCause.CLAMP) clampDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanWaveform2c.DeathCause.ACID) acidDeathAccum[bin]++;
        else if (cause == BSimReservoirPlanWaveform2c.DeathCause.OOB) oobDeathAccum[bin]++;
    }

    public VoxelReadout analyze(BSimChemicalField ahlField,
                                BSimChemicalField acidField,
                                List<BSimReservoirPlanWaveform2c.ReservoirBacterium> bacteria) {
            ...
            for (int y = 0; y < stateGrid[1]; y++) {
                for (int z = 0; z < stateGrid[2]; z++) {
                    ...
                    ahlUm[id] = ahlField.getConc(centre)
                            / BSimReservoirPlanWaveform2c.MOLECULES_PER_UM3_PER_UM;
                    pH[id] = BSimReservoirPlanWaveform2c.pHFromAcidMm(
                            BSimReservoirPlanWaveform2c.acidMmFromConc(acidField.getConc(centre)));
                }
            }
        }

        for (BSimReservoirPlanWaveform2c.ReservoirBacterium bacterium : bacteria) {
