package BSimReservoirPlanWaveform2c;
...
/**
 * Waveform2c aligned equal-histogram temporal-order classification on the
 * frozen Plan Stage 6 / HybridDish / Narma10b claim dish. Seeds 101/202/303.
 * Mechanisms are not retuned. Classes are ORDER_UNI / ORDER_DOWN / ORDER_UP.
 * Stage 6 NARMA-10 remains the provenance checkpoint. Track B is not started.
 */
public final class BSimReservoirPlanWaveform2c {
    ...
    static int NUM_WINDOWS = 400, INITIAL_POP = 1800, CARRYING_CAPACITY = 2000;
    static boolean HEADLESS = true, WRITE_SAMPLES = true, WRITE_VOXELS = true;
    static long RNG_SEED = 101;
    static Arm ARM = Arm.DRIVEN;
    static String OUTPUT_DIR = "results/waveform2c_driven_seed101";
    static String INPUT_AHL_FILE = "input_ahl_waveform2c_400.txt";
    static String INPUT_ACID_FILE = "input_acid_held05_400.txt";
    static String RUN_LABEL = "waveform2c_driven";
    ...
    static final class OutputOwner implements AutoCloseable {
        ...
        public void close() {
            ...
            try {
                ...
                throw new RuntimeException("Cannot close Waveform2c output", e);
            }
        }
    }

    public static void main(String[] args) {
        String configPath = args.length > 0 ? args[0] : "sim_config_waveform2c.properties";
        loadConfig(configPath);
        if (args.length > 1 && "preview".equals(args[1])) HEADLESS = false;
        resetStaticState();
        System.out.printf(Locale.US,
                "WAVEFORM2C dish=%.0fx%.0fx%.0f FLOW_SPEED=%.1f AHL_source=(500,250,5) "
                        + "AHL_rate=%g arm=%s seed=%d num.windows=%d%n",
        ...
        try {
            ...
            throw new RuntimeException("Cannot open Waveform2c output", e);
        }
        ...
        System.out.printf(Locale.US,
                "Waveform2c complete arm=%s summary_rows=%d expected=%d sample_rows=%d voxel_rows=%d%n",
